<?php
//Include Common Files @1-8C1D1635
define("RelativePath", "..");
...
//Class_Initialize Event @2-76419094
    function clsGridP_GLOBAL_PARAM($RelativePath, & $Parent)
    {
        ...
        $this->TYPE_I = & new clsControl(ccsLabel, "TYPE_I", "TYPE_I", ccsText, "", CCGetRequestParam("TYPE_I", ccsGet, NULL), $this);
...
//Show Method @2-2207CB07
    function Show()
    {
        ...
        if (!$this->IsEmpty) {
            ...
            $this->ControlsVisible["TYPE_I"] = $this->TYPE_I->Visible;
...
//GetErrors Method @2-D452A021
    function GetErrors()
    {
        $errors = "";
        $errors = ComposeStrings($errors, $this->CODE->Errors->ToString());
        $errors = ComposeStrings($errors, $this->VALUE->Errors->ToString());
        $errors = ComposeStrings($errors, $this->TYPE_I->Errors->ToString());
...
//DataSource Variables @2-7D856E3E
    var $Parent = "";
    var $CCSEvents = "";
    var $CCSEventResult;
    var $ErrorBlock;
...
//DataSourceClass_Initialize Event @2-D74CDD6D
...
//Open Method @2-8F3EE09A
    function Open()
    {
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildSelect", $this->Parent);
        $this->CountSQL = "SELECT COUNT(*) FROM (SELECT CODE, VALUE, TYPE_I, CASE IS_RANGE WHEN 'Y' THEN 'YES' ELSE 'NO' END AS IS_RANGE , VALUE_2 \n" .
        "FROM GLOBAL_PARAM\n" .
        "WHERE UPPER(CODE) LIKE UPPER('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%')) cnt";
        $this->SQL = "SELECT CODE, VALUE, TYPE_I, CASE IS_RANGE WHEN 'Y' THEN 'YES' ELSE 'NO' END AS IS_RANGE , VALUE_2 \n" .
        "FROM GLOBAL_PARAM\n" .
    ...
    function Operation()
    {
        ...
        if($this->Validate()) {
            if($this->PressedButton == "Button_DoSearch") {
                $Redirect = "p_global_param.php" . "?" . CCMergeQueryStrings(CCGetQueryString("Form", array("Button_DoSearch", "Button_DoSearch_x", "Button_DoSearch_y", "FLAG", "p_application_id")));
...
//Class_Initialize Event @28-90AA1C7F
    function clsRecordp_global_param1($RelativePath, & $Parent)
    {
        ...
        if($this->Visible)
        {
            ...
            $this->IS_RANGE = & new clsControl(ccsListBox, "IS_RANGE", "IS RANGE", ccsText, "", CCGetRequestParam("IS_RANGE", $Method, NULL), $this);
