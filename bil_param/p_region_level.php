<?php
//Include Common Files @1-1B424AB1
define("RelativePath", "..");
...
//Open Method @2-5E7821C3
    function Open()
    {
        ...
        "OR UPPER(LEVEL_NUMBER) LIKE UPPER('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%')) cnt";
        $this->SQL = "SELECT * FROM P_REGION_LEVEL\n" .
        "WHERE UPPER(LEVEL_NAME) LIKE UPPER('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%')\n" .
        "OR UPPER(DESCRIPTION) LIKE UPPER('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%')\n" .
        "OR UPPER(LEVEL_NUMBER) LIKE UPPER('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%')";
...
//Class_Initialize Event @32-D325F5B1
...
//CheckErrors Method @32-A0D418A2
    function CheckErrors()
    {
        $errors = false;
...
//Operation Method @32-7EC864C2
    function Operation()
    {
        ...
        if($this->PressedButton == "Button_Update") {
            ...
            $Redirect = $FileName . "?" . CCGetQueryString("QueryString", array("ccsForm", "TAMBAH", "P_REGION_LEVEL_ID", "P_REGION_LEVELPage", "s_keyword"));
            if(!CCGetEvent($this->Button_Delete->CCSEvents, "OnClick", $this->Button_Delete) || !$this->DeleteRow()) {
                $Redirect = "";
            }
        } else if($this->PressedButton == "Button_Cancel") {
            $Redirect = $FileName . "?" . CCGetQueryString("QueryString", array("ccsForm", "TAMBAH", "s_keyword", "P_REGION_LEVELPage"));
