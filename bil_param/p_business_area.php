<?php
//Include Common Files @1-3E6E02B8
define("RelativePath", "..");
...
//Open Method @2-01394CC1
    function Open()
    {
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildSelect", $this->Parent);
        $this->CountSQL = "SELECT COUNT(*) FROM (SELECT P_BUSINESS_AREA_ID, CODE, BUSINESS_AREA_NAME, case IS_BILLING_CENTER when 'Y' then 'YES' else 'NO' end as IS_BILLING_CENTER, H2H_CODE \n" .
        "FROM P_BUSINESS_AREA\n" .
        "WHERE ( UPPER(CODE) LIKE UPPER('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%')\n" .
        "OR UPPER(BUSINESS_AREA_NAME) LIKE UPPER('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%')\n" .
        "OR UPPER(H2H_CODE) LIKE UPPER('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%') \n" .
        "OR UPPER(case IS_BILLING_CENTER when 'Y' then 'YES' else 'NO' end) LIKE UPPER('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%'))) cnt";
        $this->SQL = "SELECT P_BUSINESS_AREA_ID, CODE, BUSINESS_AREA_NAME, case IS_BILLING_CENTER when 'Y' then 'YES' else 'NO' end as IS_BILLING_CENTER, H2H_CODE \n" .
        "FROM P_BUSINESS_AREA\n" .
        "WHERE ( UPPER(CODE) LIKE UPPER('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%')\n" .
        "OR UPPER(BUSINESS_AREA_NAME) LIKE UPPER('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%')\n" .
        "OR UPPER(H2H_CODE) LIKE UPPER('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%') \n" .
        "OR UPPER(case IS_BILLING_CENTER when 'Y' then 'YES' else 'NO' end) LIKE UPPER('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%'))  {SQL_OrderBy}";
...
//Class_Initialize Event @27-004A8694
    function clsRecordP_BUSINESS_AREA1($RelativePath, & $Parent)
    {
        ...
        if($this->Visible)
        {
            ...
            $this->IS_BILLING_CENTER->Values = array(array("Y", "YES"), array("N", "NO"));
...
//CheckErrors Method @27-8CC0D047
    function CheckErrors()
    {
        $errors = false;
...
//Operation Method @27-AA5AFD12
    function Operation()
    {
        ...
        if($this->PressedButton == "Button_Update") {
            ...
            $Redirect = $FileName . "?" . CCGetQueryString("QueryString", array("ccsForm", "TAMBAH", "P_APPLICATION_ID", "P_BUSINESS_AREA_ID"));
...
//DataSource Variables @27-0EBAF1F8
    var $Parent = "";
    var $CCSEvents = "";
    var $CCSEventResult;
    var $ErrorBlock;
...
//DataSourceClass_Initialize Event @27-0B34705A
