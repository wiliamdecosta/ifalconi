<?php
//Include Common Files @1-3D1A40FF
define("RelativePath", "..");
...
//Class_Initialize Event @2-6206DC86
    function clsGridP_FINANCE_PERIOD($RelativePath, & $Parent)
    {
        ...
        $this->SKUM_DATE = & new clsControl(ccsLabel, "SKUM_DATE", "SKUM_DATE", ccsDate, array("dd", "-", "mmm", "-", "yyyy"), CCGetRequestParam("SKUM_DATE", ccsGet, NULL), $this);
...
//Show Method @2-45CB01A4
    function Show()
    {
        ...
        if (!$this->IsEmpty) {
            ...
            $this->ControlsVisible["SKUM_DATE"] = $this->SKUM_DATE->Visible;
...
//GetErrors Method @2-52E01D54
    function GetErrors()
    {
        ...
        $errors = ComposeStrings($errors, $this->SKUM_DATE->Errors->ToString());
...
//DataSource Variables @2-1E0F8378
    var $Parent = "";
    var $CCSEvents = "";
    var $CCSEventResult;
    var $ErrorBlock;
...
//DataSourceClass_Initialize Event @2-93765F38
...
//SetOrder Method @2-DC485187
    function SetOrder($SorterName, $SorterDirection)
    {
        $this->Order = "a.FINANCE_PERIOD_CODE";
        $this->Order = CCGetOrder($this->Order, $SorterName, $SorterDirection, 
            "");
    }
//End SetOrder Method

//Prepare Method @2-C99858EE
    function Prepare()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
...
//Open Method @2-B22E6D4F
    function Open()
    {
        ...
        "WHERE a.P_YEAR_PERIOD_ID = " . $this->SQLValue($this->wp->GetDBValue("2"), ccsFloat) . " AND\n" .
        "(UPPER(FINANCE_PERIOD_CODE) LIKE UPPER('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%')\n" .
        "OR UPPER(b.CODE) LIKE UPPER('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%')\n" .
        "OR UPPER(a.DESCRIPTION) LIKE UPPER('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%')\n" .
        "OR UPPER(a.SKUM_NO) LIKE UPPER('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%')\n" .
        ")) cnt";
...
//Class_Initialize Event @3-0E9A2D67
...
//Class_Initialize Event @25-34CDBA82
...
//CheckErrors Method @25-B49E1C30
    function CheckErrors()
    {
        $errors = false;
...
//Operation Method @25-69AA7414
    function Operation()
    {
        ...
        if($this->PressedButton == "Button_Update") {
            ...
            $Redirect = $FileName . "?" . CCGetQueryString("QueryString", array("ccsForm", "P_FINANCE_PERIOD_ID"));
...
//DataSource Variables @25-33EA7F12
    var $Parent = "";
    var $CCSEvents = "";
    var $CCSEventResult;
    var $ErrorBlock;
...
//DataSourceClass_Initialize Event @25-34883321
...
//Open Method @25-4ACD36F4
    function Open()
    {
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildSelect", $this->Parent);
        $this->SQL = "SELECT A.*,B.CODE AS FINANCE_NAME,C.CODE AS YEAR_NAME FROM P_FINANCE_PERIOD A , P_STATUS_LIST B , P_YEAR_PERIOD C \n" .
...
//Insert Method @25-41E2E0A9
    function Insert()
    {
        ...
        $this->cp["SKUM_DATE"] = new clsSQLParameter("ctrlSKUM_DATE", ccsDate, array("dd", "-", "mmm", "-", "yyyy"), $this->DateFormat, $this->SKUM_DATE->GetValue(true), "", false, $this->ErrorBlock);
        ...
        $this->cp["P_FINANCE_PERIOD_ID"] = new clsSQLParameter("ctrlP_FINANCE_PERIOD_ID", ccsFloat, "", "", $this->P_FINANCE_PERIOD_ID->GetValue(true), "", false, $this->ErrorBlock);
        ...
        "UPPER('" . $this->SQLValue($this->cp["FINANCE_PERIOD_CODE"]->GetDBValue(), ccsText) . "'),\n" .
        ...
        "" . $this->SQLValue($this->cp["P_YEAR_PERIOD_ID"]->GetDBValue(), ccsFloat) . ",\n" .
        "'" . $this->SQLValue($this->cp["SKUM_NO"]->GetDBValue(), ccsText) . "',\n" .
        "'" . $this->SQLValue($this->cp["SKUM_DATE"]->GetDBValue(), ccsDate) . "',\n" .
...
//Update Method @25-CF83FE45
    function Update()
    {
        ...
        "SKUM_DATE = '" . $this->SQLValue($this->cp["SKUM_DATE"]->GetDBValue(), ccsDate) . "',\n" .
        "UPDATE_DATE = SYSDATE,\n" .
