...
//Class_Initialize Event @2-3A46092A
...
//DataSource Variables @2-B8A8CEC5
    var $Parent = "";
    var $CCSEvents = "";
    var $CCSEventResult;
    var $ErrorBlock;
...
//DataSourceClass_Initialize Event @2-36F05EC5
...
//Prepare Method @2-D3D675E1
    function Prepare()
    {
        ...
        $this->wp->AddParameter("2", "urlP_STATUS_TYPE_ID", ccsFloat, "", "", $this->Parameters["urlP_STATUS_TYPE_ID"], 0, false);
    }
//End Prepare Method

//Open Method @2-F5780738
    function Open()
    {
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildSelect", $this->Parent);
        $this->CountSQL = "SELECT COUNT(*) FROM (select * from P_STATUS_LIST \n" .
        "WHERE P_STATUS_TYPE_ID = " . $this->SQLValue($this->wp->GetDBValue("2"), ccsFloat) . "\n" .
        "and \n" .
        ...
        $this->SQL = "select * from P_STATUS_LIST \n" .
        "WHERE P_STATUS_TYPE_ID = " . $this->SQLValue($this->wp->GetDBValue("2"), ccsFloat) . "\n" .
        "and \n" .
        "(upper(code) like upper('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%') OR\n" .
...
//Class_Initialize Event @3-AFEF9F5C
...
//Class_Initialize Event @22-17D01402
    function clsRecordp_status_list1($RelativePath, & $Parent)
    {
        ...
        if($this->Visible)
        {
            ...
            $this->FormSubmitted = ($FormName == $this->ComponentName);
            $Method = $this->FormSubmitted ? ccsPost : ccsGet;
            $this->P_STATUS_TYPE_ID = & new clsControl(ccsTextBox, "P_STATUS_TYPE_ID", "P STATUS TYPE ID", ccsFloat, "", CCGetRequestParam("P_STATUS_TYPE_ID", $Method, NULL), $this);
...
//CheckErrors Method @22-91507C38
    function CheckErrors()
    {
        $errors = false;
        $errors = ($errors || $this->P_STATUS_TYPE_ID->Errors->Count());
...
//Operation Method @22-19E69422
    function Operation()
    {
        ...
        if($this->PressedButton == "Button_Delete") {
            $Redirect = $FileName . "?" . CCGetQueryString("QueryString", array("ccsForm", "TAMBAH", "P_STATUS_LIST_ID"));
...
//DataSource Variables @22-34A144F7
    var $Parent = "";
    var $CCSEvents = "";
    var $CCSEventResult;
    var $ErrorBlock;
...
//DataSourceClass_Initialize Event @22-9AB951E7
...
//Insert Method @22-E09449CD
    function Insert()
    {
        ...
        $this->SQL = "INSERT INTO P_STATUS_LIST(P_STATUS_LIST_ID,P_STATUS_TYPE_ID, CODE, LISTING_NO,DESCRIPTION,UPDATE_DATE, UPDATE_BY)\n" .
        "VALUES\n" .
        "(GENERATE_ID('BILLDB','P_STATUS_LIST','P_STATUS_LIST_ID')," . $this->SQLValue($this->cp["P_STATUS_TYPE_ID"]->GetDBValue(), ccsFloat) . ",UPPER('" . $this->SQLValue($this->cp["CODE"]->GetDBValue(), ccsText) . "')," . $this->SQLValue($this->cp["LISTING_NO"]->GetDBValue(), ccsFloat) . ",INITCAP(TRIM('" . $this->SQLValue($this->cp["DESCRIPTION"]->GetDBValue(), ccsText) . "')),sysdate, '" . $this->SQLValue($this->cp["UPDATE_BY"]->GetDBValue(), ccsText) . "')";
