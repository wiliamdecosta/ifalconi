<?php
//Include Common Files @1-4098D475
define("RelativePath", "..");
define("PathToCurrentPage", "/bil_param/");
define("FileName", "p_bundled_feature.php");
...
class clsGridV_P_BUNDLED_FEATURE { //V_P_BUNDLED_FEATURE class @2-77A23E8E
...
//Class_Initialize Event @2-BC3B37CE
    function clsGridV_P_BUNDLED_FEATURE($RelativePath, & $Parent)
    {
        global $FileName;
        global $CCSLocales;
        global $DefaultDateFormat;
        $this->ComponentName = "V_P_BUNDLED_FEATURE";
        $this->Visible = True;
        $this->Parent = & $Parent;
        $this->RelativePath = $RelativePath;
        $this->Errors = new clsErrors();
        $this->ErrorBlock = "Grid V_P_BUNDLED_FEATURE";
        $this->Attributes = new clsAttributes($this->ComponentName . ":");
        $this->DataSource = new clsV_P_BUNDLED_FEATUREDataSource($this);
        ...
        $this->DLink->Page = "p_bundled_feature.php";
        ...
        $this->ST_NEW->Page = "p_bundled_feature.php";
    }
    ...
    function Show()
    {
        ...
        if (!$this->IsEmpty) {
            ...
            while ($this->ForceIteration || (($this->RowNumber < $this->PageSize) &&  ($this->HasRecord = $this->DataSource->has_next_record()))) {
                ...
                $this->DLink->Parameters = CCAddParam($this->DLink->Parameters, "P_BUNDLED_FEATURE_ID", $this->DataSource->f("P_BUNDLED_FEATURE_ID"));
        ...
        $this->ST_NEW->Parameters = CCGetQueryString("QueryString", array("P_BUNDLED_FEATURE_ID", "ccsForm"));
...
} //End V_P_BUNDLED_FEATURE Class @2-FCB6E20C

class clsV_P_BUNDLED_FEATUREDataSource extends clsDBConn {  //V_P_BUNDLED_FEATUREDataSource Class @2-7C56984D

//DataSource Variables @2-7C5E10D5
    var $Parent = "";
    var $CCSEvents = "";
    var $CCSEventResult;
    var $ErrorBlock;
...
//DataSourceClass_Initialize Event @2-34FAA757
    function clsV_P_BUNDLED_FEATUREDataSource(& $Parent)
    {
        $this->Parent = & $Parent;
        $this->ErrorBlock = "Grid V_P_BUNDLED_FEATURE";
        $this->Initialize();
...
//Open Method @2-1DD2483E
    function Open()
    {
        ...
        "OR UPPER(SERVICE_TYPE_CODE) LIKE UPPER('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%')\n" .
        "OR UPPER(TARIFF_LOCATION_CODE) LIKE UPPER('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%')\n" .
...
} //End V_P_BUNDLED_FEATUREDataSource Class @2-FCB6E20C
...
//Class_Initialize Event @3-A930BB18
...
//Operation Method @3-A01C32CE
    function Operation()
    {
        ...
        $Redirect = "p_bundled_feature.php";
        if($this->Validate()) {
            if($this->PressedButton == "Button_DoSearch") {
                $Redirect = "p_bundled_feature.php" . "?" . CCMergeQueryStrings(CCGetQueryString("Form", array("Button_DoSearch", "Button_DoSearch_x", "Button_DoSearch_y")));
...
//Class_Initialize Event @39-472CDA76
    function clsRecordV_P_BUNDLED_FEATURE1($RelativePath, & $Parent)
    {
        ...
        if($this->Visible)
        {
            ...
            $this->TARIFF_LOCATION_ID = & new clsControl(ccsTextBox, "TARIFF_LOCATION_ID", "TARIFF_LOCATION_ID", ccsFloat, "", CCGetRequestParam("TARIFF_LOCATION_ID", $Method, NULL), $this);
            ...
            $this->P_BUNDLED_FEATURE_ID = & new clsControl(ccsHidden, "P_BUNDLED_FEATURE_ID", "P_BUNDLED_FEATURE_ID", ccsText, "", CCGetRequestParam("P_BUNDLED_FEATURE_ID", $Method, NULL), $this);
            ...
            $this->TARIFF_LOCATION_CODE = & new clsControl(ccsTextBox, "TARIFF_LOCATION_CODE", "Tarif Location Code", ccsText, "", CCGetRequestParam("TARIFF_LOCATION_CODE", $Method, NULL), $this);
...
//Validate Method @39-3360A525
    function Validate()
    {
        ...
        $Validation = ($this->TARIFF_LOCATION_ID->Validate() && $Validation);
        ...
        $Validation =  $Validation && ($this->TARIFF_LOCATION_ID->Errors->Count() == 0);
...
//CheckErrors Method @39-A6B6C4DF
    function CheckErrors()
    {
        $errors = false;
        $errors = ($errors || $this->CODE->Errors->Count());
        $errors = ($errors || $this->P_SERVICE_TYPE_ID->Errors->Count());
        $errors = ($errors || $this->TARIFF_LOCATION_ID->Errors->Count());
...
//Operation Method @39-C892F7BD
    function Operation()
    {
        ...
        if($this->PressedButton == "Button_Delete") {
            $Redirect = $FileName . "?" . CCGetQueryString("QueryString", array("ccsForm", "P_BUNDLED_FEATURE_ID"));
...
//InsertRow Method @39-96FD9CD3
    function InsertRow()
    {
        ...
        $this->DataSource->TARIFF_LOCATION_ID->SetValue($this->TARIFF_LOCATION_ID->GetValue(true));
...
//Show Method @39-E54FE720
    function Show()
    {
        ...
        if($this->EditMode) {
            ...
            if($this->DataSource->Errors->Count() == 0 && $this->DataSource->next_record()) {
                ...
                if(!$this->FormSubmitted){
                    ...
                    $this->TARIFF_LOCATION_ID->SetValue($this->DataSource->TARIFF_LOCATION_ID->GetValue());
                    $this->VALID_FROM->SetValue($this->DataSource->VALID_FROM->GetValue());
        ...
        if($this->FormSubmitted || $this->CheckErrors()) {
            ...
            $Error = ComposeStrings($Error, $this->TARIFF_LOCATION_ID->Errors->ToString());
...
//DataSource Variables @39-FBA292C8
    var $Parent = "";
    var $CCSEvents = "";
    var $CCSEventResult;
    var $ErrorBlock;
...
//DataSourceClass_Initialize Event @39-48875C61
    function clsV_P_BUNDLED_FEATURE1DataSource(& $Parent)
    {
        ...
        $this->TARIFF_LOCATION_ID = new clsField("TARIFF_LOCATION_ID", ccsFloat, "");
...
//Prepare Method @39-2CF4544A
    function Prepare()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
        $this->wp = new clsSQLParameters($this->ErrorBlock);
        $this->wp->AddParameter("1", "urlP_BUNDLED_FEATURE_ID", ccsFloat, "", "", $this->Parameters["urlP_BUNDLED_FEATURE_ID"], "", false);
        $this->AllParametersSet = $this->wp->AllParamsSet();
        $this->wp->Criterion[1] = $this->wp->Operation(opEqual, "P_BUNDLED_FEATURE_ID", $this->wp->GetDBValue("1"), $this->ToSQL($this->wp->GetDBValue("1"), ccsFloat),false);
        $this->Where = 
             $this->wp->Criterion[1];
    }
//End Prepare Method

//Open Method @39-2D604640
    function Open()
    {
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildSelect", $this->Parent);
        $this->SQL = "SELECT * \n\n" .
        "FROM V_P_BUNDLED_FEATURE {SQL_Where} {SQL_OrderBy}";
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeExecuteSelect", $this->Parent);
        $this->query(CCBuildSQL($this->SQL, $this->Where, $this->Order));
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "AfterExecuteSelect", $this->Parent);
    }
...
//SetValues Method @39-C3104531
    function SetValues()
    {
        $this->CODE->SetDBValue($this->f("CODE"));
        $this->P_SERVICE_TYPE_ID->SetDBValue(trim($this->f("P_SERVICE_TYPE_ID")));
        $this->TARIFF_LOCATION_ID->SetDBValue(trim($this->f("TARIFF_LOCATION_ID")));
        $this->VALID_FROM->SetDBValue(trim($this->f("VALID_FROM")));
...
//Insert Method @39-F59D2355
    function Insert()
    {
        ...
        $this->cp["TARIFF_LOCATION_ID"] = new clsSQLParameter("ctrlTARIFF_LOCATION_ID", ccsFloat, "", "", $this->TARIFF_LOCATION_ID->GetValue(true), "", false, $this->ErrorBlock);
        ...
        if (!is_null($this->cp["TARIFF_LOCATION_ID"]->GetValue()) and !strlen($this->cp["TARIFF_LOCATION_ID"]->GetText()) and !is_bool($this->cp["TARIFF_LOCATION_ID"]->GetValue())) 
            $this->cp["TARIFF_LOCATION_ID"]->SetValue($this->TARIFF_LOCATION_ID->GetValue(true));
        $this->SQL = "INSERT INTO P_BUNDLED_FEATURE(\n" .
        ...
        "" . $this->SQLValue($this->cp["TARIFF_LOCATION_ID"]->GetDBValue(), ccsFloat) . ", \n" .
...
//Update Method @39-65F07467
    function Update()
    {
        ...
        $this->SQL = "UPDATE P_BUNDLED_FEATURE SET \n" .
        ...
        "TARIFF_LOCATION_ID=" . $this->SQLValue($this->cp["TARIFF_LOCATION_ID"]->GetDBValue(), ccsFloat) . ", \n" .
...
//Delete Method @39-3E2ED8FD
    function Delete()
    {
        ...
        if (!strlen($this->cp["P_BUNDLED_FEATURE_ID"]->GetText()) and !is_bool($this->cp["P_BUNDLED_FEATURE_ID"]->GetValue(true))) 
            $this->cp["P_BUNDLED_FEATURE_ID"]->SetText(0);
        $this->SQL = "DELETE FROM P_BUNDLED_FEATURE WHERE  P_BUNDLED_FEATURE_ID= " . $this->SQLValue($this->cp["P_BUNDLED_FEATURE_ID"]->GetDBValue(), ccsFloat) . "";
...
$FileName = FileName;
$Redirect = "";
$TemplateFileName = "p_bundled_feature.html";
$BlockToParse = "main";
...
//Include events file @1-A0BA0F79
include_once("./p_bundled_feature_events.php");
...
// Controls
$V_P_BUNDLED_FEATURE = & new clsGridV_P_BUNDLED_FEATURE("", $MainPage);
...
//Execute Components @1-9613B27F
$V_P_SERVICE_CATEGORYSearch->Operation();
$V_P_BUNDLED_FEATURE1->Operation();
//End Execute Components

//Go to destination page @1-7535667F
if($Redirect)
{
    $CCSEventResult = CCGetEvent($CCSEvents, "BeforeUnload", $MainPage);
    $DBConn->close();
    header("Location: " . $Redirect);
    unset($V_P_BUNDLED_FEATURE);
...
//Show Page @1-A805C34C
$V_P_BUNDLED_FEATURE->Show();
