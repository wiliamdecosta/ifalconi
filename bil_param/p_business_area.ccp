<Page id="1" templateExtension="html" relativePath=".." fullRelativePath=".\bil_param" secured="False" urlType="Relative" isIncluded="False" SSLAccess="False" isService="False" cachingEnabled="False" cachingDuration="1 minutes" wizardTheme="Apricot" wizardThemeVersion="3.0" needGeneration="0">
	<Components>
		<Grid id="2" secured="False" sourceType="SQL" returnValueType="Number" defaultPageSize="10" connection="Conn" dataSource="SELECT P_BUSINESS_AREA_ID, CODE, BUSINESS_AREA_NAME, case IS_BILLING_CENTER when 'Y' then 'YES' else 'NO' end as IS_BILLING_CENTER, H2H_CODE 
FROM P_BUSINESS_AREA
WHERE ( UPPER(CODE) LIKE UPPER('%{s_keyword}%')
OR UPPER(BUSINESS_AREA_NAME) LIKE UPPER('%{s_keyword}%')
OR UPPER(H2H_CODE) LIKE UPPER('%{s_keyword}%') 
OR UPPER(case IS_BILLING_CENTER when 'Y' then 'YES' else 'NO' end) LIKE UPPER('%{s_keyword}%')) 
ORDER BY P_BUSINESS_AREA_ID" name="P_BUSINESS_AREA" orderBy="P_BUSINESS_AREA_ID" pageSizeLimit="100" wizardCaption=" P BUSINESS AREA " wizardGridType="Tabular" wizardSortingType="SimpleDir" wizardAllowInsert="True" wizardAltRecord="False" wizardAltRecordType="Style" wizardRecordSeparator="False" wizardNoRecords="no records" pasteActions="pasteActions">
			<Components>
			...
			<SPParameters/>
			<SQLParameters>
				<SQLParameter id="96" parameterType="URL" variable="s_keyword" dataType="Text" parameterSource="s_keyword"/>
			</SQLParameters>
			<SecurityGroups/>
			<Attributes/>
			<Features/>
...
VALUES (GENERATE_ID('BILLDB','P_BUSINESS_AREA','P_BUSINESS_AREA_ID'),'{CODE}','{BUSINESS_AREA_NAME}','{IS_BILLING_CENTER}','{OFFICE_ADDRESS}','{H2H_CODE}','{DESCRIPTION}',SYSDATE,'{UPDATE_BY}')" pasteAsReplace="pasteAsReplace">
			<Components>
				<TextBox id="34" visible="Yes" fieldSourceType="DBColumn" dataType="Text" name="CODE" fieldSource="CODE" required="True" caption="CODE" wizardCaption="CODE" wizardSize="6" wizardMaxLength="6" wizardIsPassword="False" wizardUseTemplateBlock="False" PathID="P_BUSINESS_AREA1CODE">
					<Components/>
					<Events/>
					<Attributes/>
					<Features/>
				...
				<Button id="45" urlType="Relative" enableValidation="False" isDefault="False" name="Button_Delete" operation="Delete" wizardCaption="Delete" PathID="P_BUSINESS_AREA1Button_Delete" wizardTheme="sikm" wizardThemeVersion="3.0" removeParameters="TAMBAH;P_APPLICATION_ID;P_BUSINESS_AREA_ID">
					<Components/>
					<Events>
						<Event name="OnClick" type="Client">
							<Actions>
								<Action actionName="Confirmation Message" actionCategory="General" id="46" eventType="Client" message="Are you sure want to delete this record ?"/>
							</Actions>
						</Event>
					</Events>
					<Attributes/>
