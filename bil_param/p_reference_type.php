...
//Class_Initialize Event @2-F2727707
...
//Open Method @2-B3B76560
    function Open()
    {
        ...
        "OR UPPER(REFERENCE_NAME) LIKE UPPER('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%') )) cnt";
        $this->SQL = "SELECT * \n" .
        "FROM P_REFERENCE_TYPE\n" .
        "WHERE ( UPPER(CODE) LIKE UPPER('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%')\n" .
        "OR UPPER(REFERENCE_NAME) LIKE UPPER('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%') )  {SQL_OrderBy}";
...
//Class_Initialize Event @35-997660CA
...
//CheckErrors Method @35-448D72C6
    function CheckErrors()
    {
        $errors = false;
...
//DataSource Variables @35-4AA75444
    var $Parent = "";
    var $CCSEvents = "";
    var $CCSEventResult;
    var $ErrorBlock;
...
//DataSourceClass_Initialize Event @35-1A7AD1DB
...
//Insert Method @35-F426BF71
    function Insert()
    {
        ...
        $this->SQL = "INSERT INTO P_REFERENCE_TYPE (P_REFERENCE_TYPE_ID,CODE,REFERENCE_NAME,LISTING_NO,DESCRIPTION, UPDATE_DATE, UPDATE_BY) \n" .
        "VALUES\n" .
        "(GENERATE_ID('BILLDB','P_REFERENCE_TYPE','P_REFERENCE_TYPE_ID'),UPPER(TRIM('" . $this->SQLValue($this->cp["CODE"]->GetDBValue(), ccsText) . "')),UPPER(TRIM('" . $this->SQLValue($this->cp["REFERENCE_NAME"]->GetDBValue(), ccsText) . "'))," . $this->SQLValue($this->cp["LISTING_NO"]->GetDBValue(), ccsFloat) . ",INITCAP(TRIM('" . $this->SQLValue($this->cp["DESCRIPTION"]->GetDBValue(), ccsText) . "')), sysdate, '" . $this->SQLValue($this->cp["UPDATE_BY"]->GetDBValue(), ccsText) . "')";
