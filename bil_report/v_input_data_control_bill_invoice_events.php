<?php
//BindEvents Method @1-C0CCDFC1
function BindEvents()
{
    global $v_input_data_control_bill_invoiceGrid;
    global $CCSEvents;
    $v_input_data_control_bill_invoiceGrid->CCSEvents["BeforeShowRow"] = "v_input_data_control_bill_invoiceGrid_BeforeShowRow";
...
//v_input_data_control_bill_invoiceGrid_BeforeShowRow @282-2A9F8280
function v_input_data_control_bill_invoiceGrid_BeforeShowRow(& $sender)
{
    $v_input_data_control_bill_invoiceGrid_BeforeShowRow = true;
    $Component = & $sender;
    $Container = & CCGetParentContainer($sender);
    global $v_input_data_control_bill_invoiceGrid; //Compatibility
//End v_input_data_control_bill_invoiceGrid_BeforeShowRow
...
//DEL      // Write your own code here.
	...
	if (empty($keyId)) {
		if (empty($id)) {
			$id = $v_input_data_control_bill_invoiceGrid->INPUT_DATA_CONTROL_ID->GetValue();
		}
		global $FileName;
	...
	if ($v_input_data_control_bill_invoiceGrid->INPUT_DATA_CONTROL_ID->GetValue() == $keyId) {
		$v_input_data_control_bill_invoiceGrid->IDCid->SetValue($keyId);
		$v_input_data_control_bill_invoiceGrid->ADLink->Visible = true;
		$v_input_data_control_bill_invoiceGrid->DLink->Visible = false;
		$Component->Attributes->SetValue("rowStyle", "class=AltRow");
	} else {
		$v_input_data_control_bill_invoiceGrid->ADLink->Visible = false;
		$v_input_data_control_bill_invoiceGrid->DLink->Visible = true;
		$Component->Attributes->SetValue("rowStyle", "class=Row");
	}
//DEL  // -------------------------

//Close v_input_data_control_bill_invoiceGrid_BeforeShowRow @282-5F3DCF80
    return $v_input_data_control_bill_invoiceGrid_BeforeShowRow;
}
//End Close v_input_data_control_bill_invoiceGrid_BeforeShowRow

//Page_BeforeShow @1-6B211B86
function Page_BeforeShow(& $sender)
{
    $Page_BeforeShow = true;
    $Component = & $sender;
    $Container = & CCGetParentContainer($sender);
    global $v_input_data_control_bill_invoice; //Compatibility
//End Page_BeforeShow
