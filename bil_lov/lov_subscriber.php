<?php
//Include Common Files @1-8CB95049
define("RelativePath", "..");
define("PathToCurrentPage", "/bil_lov/");
define("FileName", "lov_subscriber.php");
...
class clsGridSUBSCRIBER { //SUBSCRIBER class @2-FA72E295
...
//Class_Initialize Event @2-068E164A
    function clsGridSUBSCRIBER($RelativePath, & $Parent)
    {
        global $FileName;
        global $CCSLocales;
        ...
        $this->ErrorBlock = "Grid SUBSCRIBER";
        $this->Attributes = new clsAttributes($this->ComponentName . ":");
        $this->DataSource = new clsSUBSCRIBERDataSource($this);
...
} //End SUBSCRIBER Class @2-FCB6E20C

class clsSUBSCRIBERDataSource extends clsDBConn {  //SUBSCRIBERDataSource Class @2-62C44FA5

//DataSource Variables @2-10254261
    var $Parent = "";
    var $CCSEvents = "";
    var $CCSEventResult;
    var $ErrorBlock;
...
//DataSourceClass_Initialize Event @2-FEEA5E7D
    function clsSUBSCRIBERDataSource(& $Parent)
    {
        $this->Parent = & $Parent;
        $this->ErrorBlock = "Grid SUBSCRIBER";
        $this->Initialize();
...
//Open Method @2-8C05128A
    function Open()
    {
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildSelect", $this->Parent);
        $this->CountSQL = "SELECT COUNT(*) FROM (SELECT a.SERVICE_NO, a.SUBSCRIPTION_NAME, b.CODE SERVICE_TYPE_CODE, a.SUBSCRIBER_ID\n" .
        "FROM SUBSCRIBER a, P_SERVICE_TYPE b\n" .
        "WHERE a.P_SERVICE_TYPE_ID = b.P_SERVICE_TYPE_ID AND\n" .
        "	(UPPER(a.SERVICE_NO) LIKE UPPER('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%') OR\n" .
        "	UPPER(a.SUBSCRIPTION_NAME) LIKE UPPER('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%') OR\n" .
        "	UPPER(b.CODE) LIKE UPPER('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%'))\n" .
        ") cnt";
        $this->SQL = "SELECT a.SERVICE_NO, a.SUBSCRIPTION_NAME, b.CODE SERVICE_TYPE_CODE, a.SUBSCRIBER_ID\n" .
        "FROM SUBSCRIBER a, P_SERVICE_TYPE b\n" .
        "WHERE a.P_SERVICE_TYPE_ID = b.P_SERVICE_TYPE_ID AND\n" .
        "	(UPPER(a.SERVICE_NO) LIKE UPPER('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%') OR\n" .
        "	UPPER(a.SUBSCRIPTION_NAME) LIKE UPPER('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%') OR\n" .
...
} //End SUBSCRIBERDataSource Class @2-FCB6E20C
...
//Class_Initialize Event @3-6422986B
...
//Operation Method @3-E16D3A69
    function Operation()
    {
        ...
        $Redirect = "lov_subscriber.php";
        if($this->Validate()) {
            if($this->PressedButton == "Button_DoSearch") {
                $Redirect = "lov_subscriber.php" . "?" . CCMergeQueryStrings(CCGetQueryString("Form", array("Button_DoSearch", "Button_DoSearch_x", "Button_DoSearch_y", "TAMBAH", "BATCH_CONTROL_ID")));
...
$FileName = FileName;
$Redirect = "";
$TemplateFileName = "lov_subscriber.html";
$BlockToParse = "main";
...
//Include events file @1-95EAD45A
include_once("./lov_subscriber_events.php");
...
// Controls
$SUBSCRIBER = & new clsGridSUBSCRIBER("", $MainPage);
$P_COMPANYSearch = & new clsRecordP_COMPANYSearch("", $MainPage);
$MainPage->SUBSCRIBER = & $SUBSCRIBER;
...
//Go to destination page @1-ABB14C4C
if($Redirect)
{
