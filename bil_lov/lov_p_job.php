<?php
//Include Common Files @1-FBF972AE
define("RelativePath", "..");
...
//Class_Initialize Event @2-8F717994
...
//DataSource Variables @2-A2D53AFB
    var $Parent = "";
    var $CCSEvents = "";
    var $CCSEventResult;
    var $ErrorBlock;
...
//DataSourceClass_Initialize Event @2-254C00FA
...
//Open Method @2-11E56E0F
    function Open()
    {
        ...
        "OR UPPER(PROCEDURE_NAME) LIKE UPPER('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%') )) cnt";
        $this->SQL = "SELECT * \n" .
        "FROM P_JOB\n" .
        "WHERE ( UPPER(CODE) LIKE UPPER('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%')\n" .
        "OR UPPER(PROCEDURE_NAME) LIKE UPPER('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%') )  {SQL_OrderBy}";
...
//Operation Method @3-04303A21
    function Operation()
    {
        ...
        $Redirect = "lov_p_job.php";
        if($this->Validate()) {
            if($this->PressedButton == "Button_DoSearch") {
                $Redirect = "lov_p_job.php" . "?" . CCMergeQueryStrings(CCGetQueryString("Form", array("Button_DoSearch", "Button_DoSearch_x", "Button_DoSearch_y", "FLAG", "p_application_id")));
