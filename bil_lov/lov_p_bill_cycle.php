<?php
//Include Common Files @1-844FCD6A
define("RelativePath", "..");
define("PathToCurrentPage", "/bil_lov/");
define("FileName", "lov_p_bill_cycle.php");
...
//Class_Initialize Event @2-5ADF9550
...
//DataSource Variables @2-C0BB3C37
    var $Parent = "";
    var $CCSEvents = "";
    var $CCSEventResult;
    var $ErrorBlock;
...
//DataSourceClass_Initialize Event @2-3A783192
...
//Open Method @2-5D0038EB
    function Open()
    {
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildSelect", $this->Parent);
        $this->CountSQL = "SELECT COUNT(*) FROM (SELECT P_BILL_CYCLE_ID,CODE FROM P_BILL_CYCLE WHERE UPPER(CODE)LIKE UPPER('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%')) cnt";
        $this->SQL = "SELECT P_BILL_CYCLE_ID,CODE FROM P_BILL_CYCLE WHERE UPPER(CODE)LIKE UPPER('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%')";
...
//Operation Method @3-D4747D86
    function Operation()
    {
        ...
        $Redirect = "lov_p_bill_cycle.php";
        if($this->Validate()) {
            if($this->PressedButton == "Button_DoSearch") {
                $Redirect = "lov_p_bill_cycle.php" . "?" . CCMergeQueryStrings(CCGetQueryString("Form", array("Button_DoSearch", "Button_DoSearch_x", "Button_DoSearch_y", "FLAG", "p_application_id")));
...
$FileName = FileName;
$Redirect = "";
$TemplateFileName = "lov_p_bill_cycle.html";
$BlockToParse = "main";
...
//Include events file @1-DD972DB8
include_once("./lov_p_bill_cycle_events.php");
...
//Go to destination page @1-08336F7C
if($Redirect)
{
