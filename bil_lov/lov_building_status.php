<?php
//Include Common Files @1-D40752F3
define("RelativePath", "..");
define("PathToCurrentPage", "/bil_lov/");
define("FileName", "lov_building_status.php");
...
class clsGridBUILDING_STATUS { //BUILDING_STATUS class @2-7FAC423C
...
//Class_Initialize Event @2-75FE2298
    function clsGridBUILDING_STATUS($RelativePath, & $Parent)
    {
        global $FileName;
        global $CCSLocales;
        global $DefaultDateFormat;
        $this->ComponentName = "BUILDING_STATUS";
        $this->Visible = True;
        $this->Parent = & $Parent;
        $this->RelativePath = $RelativePath;
        $this->Errors = new clsErrors();
        $this->ErrorBlock = "Grid BUILDING_STATUS";
        $this->Attributes = new clsAttributes($this->ComponentName . ":");
        $this->DataSource = new clsBUILDING_STATUSDataSource($this);
...
} //End BUILDING_STATUS Class @2-FCB6E20C

class clsBUILDING_STATUSDataSource extends clsDBConn {  //BUILDING_STATUSDataSource Class @2-891BDD7E
...
//DataSourceClass_Initialize Event @2-C78558B7
    function clsBUILDING_STATUSDataSource(& $Parent)
    {
        $this->Parent = & $Parent;
        $this->ErrorBlock = "Grid BUILDING_STATUS";
        $this->Initialize();
...
//Open Method @2-720CFDAE
    function Open()
    {
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildSelect", $this->Parent);
        $this->CountSQL = "SELECT COUNT(*) FROM (SELECT * FROM P_REFERENCE_LIST WHERE P_REFERENCE_TYPE_ID=7 AND UPPER(CODE)LIKE UPPER('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%')) cnt";
        $this->SQL = "SELECT * FROM P_REFERENCE_LIST WHERE P_REFERENCE_TYPE_ID=7 AND UPPER(CODE)LIKE UPPER('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%')";
...
} //End BUILDING_STATUSDataSource Class @2-FCB6E20C
...
//Operation Method @3-2D00B0C1
    function Operation()
    {
        ...
        $Redirect = "lov_building_status.php";
        if($this->Validate()) {
            if($this->PressedButton == "Button_DoSearch") {
                $Redirect = "lov_building_status.php" . "?" . CCMergeQueryStrings(CCGetQueryString("Form", array("Button_DoSearch", "Button_DoSearch_x", "Button_DoSearch_y", "FLAG", "p_application_id")));
...
$FileName = FileName;
$Redirect = "";
$TemplateFileName = "lov_building_status.html";
$BlockToParse = "main";
...
//Include events file @1-638B9300
include_once("./lov_building_status_events.php");
...
// Controls
$BUILDING_STATUS = & new clsGridBUILDING_STATUS("", $MainPage);
$P_SERVICE_TYPESearch = & new clsRecordP_SERVICE_TYPESearch("", $MainPage);
$MainPage->BUILDING_STATUS = & $BUILDING_STATUS;
$MainPage->P_SERVICE_TYPESearch = & $P_SERVICE_TYPESearch;
$BUILDING_STATUS->Initialize();
...
//Go to destination page @1-50064167
if($Redirect)
{
    $CCSEventResult = CCGetEvent($CCSEvents, "BeforeUnload", $MainPage);
    $DBConn->close();
    header("Location: " . $Redirect);
    unset($BUILDING_STATUS);
