<?php
//Include Common Files @1-AA1BB2A6
define("RelativePath", "..");
define("PathToCurrentPage", "/bil_lov/");
define("FileName", "lov_ticket_reason.php");
...
//Class_Initialize Event @2-1F3EA22E
...
//DataSourceClass_Initialize Event @2-CE215A61
...
//Open Method @2-2258AA86
    function Open()
    {
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildSelect", $this->Parent);
        $this->CountSQL = "SELECT COUNT(*) FROM (SELECT CODE, P_REFERENCE_LIST_ID\n" .
        "FROM P_REFERENCE_LIST\n" .
        "WHERE P_REFERENCE_TYPE_ID=9 AND UPPER(CODE) LIKE UPPER('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%')) cnt";
        $this->SQL = "SELECT CODE, P_REFERENCE_LIST_ID\n" .
        "FROM P_REFERENCE_LIST\n" .
        "WHERE P_REFERENCE_TYPE_ID=9 AND UPPER(CODE) LIKE UPPER('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%') {SQL_OrderBy}";
...
//Operation Method @3-12E6E9C7
    function Operation()
    {
        ...
        $Redirect = "lov_ticket_reason.php";
        if($this->Validate()) {
            if($this->PressedButton == "Button_DoSearch") {
                $Redirect = "lov_ticket_reason.php" . "?" . CCMergeQueryStrings(CCGetQueryString("Form", array("Button_DoSearch", "Button_DoSearch_x", "Button_DoSearch_y", "TAMBAH", "BATCH_CONTROL_ID")));
...
$FileName = FileName;
$Redirect = "";
$TemplateFileName = "lov_ticket_reason.html";
$BlockToParse = "main";
...
//Include events file @1-6265344D
include_once("./lov_ticket_reason_events.php");
