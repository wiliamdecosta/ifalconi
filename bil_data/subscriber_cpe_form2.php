<?php
//Include Common Files @1-E599DDB3
define("RelativePath", "..");
define("PathToCurrentPage", "/bil_data/");
define("FileName", "subscriber_cpe_form2.php");
...
class clsRecordCPE_RECURRING_TARIFF { //CPE_RECURRING_TARIFF Class @110-A09F8132
...
//Class_Initialize Event @110-F6274DE6
    function clsRecordCPE_RECURRING_TARIFF($RelativePath, & $Parent)
    {
        ...
        $this->RelativePath = $RelativePath;
        $this->Errors = new clsErrors();
        $this->ErrorBlock = "Record CPE_RECURRING_TARIFF/Error";
        $this->DataSource = new clsCPE_RECURRING_TARIFFDataSource($this);
        $this->ds = & $this->DataSource;
        $this->InsertAllowed = true;
        $this->UpdateAllowed = true;
        $this->DeleteAllowed = true;
        ...
        if($this->Visible)
        {
            $this->ComponentName = "CPE_RECURRING_TARIFF";
            $this->Attributes = new clsAttributes($this->ComponentName . ":");
            $CCSForm = explode(":", CCGetFromGet("ccsForm", ""), 2);
            if(sizeof($CCSForm) == 1)
                $CCSForm[1] = "";
            ...
            $this->DatePicker_VALID_FROM = & new clsDatePicker("DatePicker_VALID_FROM", "CPE_RECURRING_TARIFF", "VALID_FROM", $this);
            ...
            $this->DatePicker_VALID_TO = & new clsDatePicker("DatePicker_VALID_TO", "CPE_RECURRING_TARIFF", "VALID_TO", $this);
            ...
            $this->SUBSCRIBER_CPE_TARIFF_ID = & new clsControl(ccsHidden, "SUBSCRIBER_CPE_TARIFF_ID", "SUBSCRIBER_CPE_TARIFF_ID", ccsFloat, "", CCGetRequestParam("SUBSCRIBER_CPE_TARIFF_ID", $Method, NULL), $this);
            $this->SUBSCRIBER_CPE_ID = & new clsControl(ccsTextBox, "SUBSCRIBER_CPE_ID", "SUBSCRIBER_CPE_ID", ccsFloat, "", CCGetRequestParam("SUBSCRIBER_CPE_ID", $Method, NULL), $this);
            if(!$this->FormSubmitted) {
...
//Initialize Method @110-E2B98060
    function Initialize()
    {
        ...
        $this->DataSource->Parameters["urlSUBSCRIBER_CPE_TARIFF_ID"] = CCGetFromGet("SUBSCRIBER_CPE_TARIFF_ID", NULL);
    }
//End Initialize Method

//Validate Method @110-72B162F3
    function Validate()
    {
        global $CCSLocales;
        $Validation = true;
        ...
        $Validation = ($this->SUBSCRIBER_CPE_TARIFF_ID->Validate() && $Validation);
        ...
        $Validation =  $Validation && ($this->SUBSCRIBER_CPE_TARIFF_ID->Errors->Count() == 0);
        $Validation =  $Validation && ($this->SUBSCRIBER_CPE_ID->Errors->Count() == 0);
        return (($this->Errors->Count() == 0) && $Validation);
    }
//End Validate Method

//CheckErrors Method @110-3C05E071
    function CheckErrors()
    {
        $errors = false;
        ...
        $errors = ($errors || $this->SUBSCRIBER_CPE_TARIFF_ID->Errors->Count());
...
//Operation Method @110-E35435BF
    function Operation()
    {
        ...
        if($this->PressedButton == "Button_Delete") {
            $Redirect = "subscriber_cpe.php" . "?" . CCGetQueryString("QueryString", array("ccsForm", "SUBSCRIBER_CPE_ID", "SUBSCRIBER_CPE_ID"));
...
//InsertRow Method @110-F2977754
    function InsertRow()
    {
        ...
        $this->DataSource->SUBSCRIBER_CPE_TARIFF_ID->SetValue($this->SUBSCRIBER_CPE_TARIFF_ID->GetValue(true));
...
//Show Method @110-91F49C81
    function Show()
    {
        ...
        if($this->FormSubmitted || $this->CheckErrors()) {
            ...
            $Error = ComposeStrings($Error, $this->SUBSCRIBER_CPE_TARIFF_ID->Errors->ToString());
...
} //End CPE_RECURRING_TARIFF Class @110-FCB6E20C

class clsCPE_RECURRING_TARIFFDataSource extends clsDBConn {  //CPE_RECURRING_TARIFFDataSource Class @110-5F117831

//DataSource Variables @110-1C370C4E
    var $Parent = "";
    var $CCSEvents = "";
    var $CCSEventResult;
    var $ErrorBlock;
...
//DataSourceClass_Initialize Event @110-B98CF826
    function clsCPE_RECURRING_TARIFFDataSource(& $Parent)
    {
        $this->Parent = & $Parent;
        $this->ErrorBlock = "Record CPE_RECURRING_TARIFF/Error";
        $this->Initialize();
...
//Prepare Method @110-B97D059F
    function Prepare()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
        $this->wp = new clsSQLParameters($this->ErrorBlock);
        $this->wp->AddParameter("1", "urlSUBSCRIBER_CPE_TARIFF_ID", ccsFloat, "", "", $this->Parameters["urlSUBSCRIBER_CPE_TARIFF_ID"], "", false);
        $this->AllParametersSet = $this->wp->AllParamsSet();
        $this->wp->Criterion[1] = $this->wp->Operation(opEqual, "SUBSCRIBER_CPE_TARIFF_ID", $this->wp->GetDBValue("1"), $this->ToSQL($this->wp->GetDBValue("1"), ccsFloat),false);
        $this->Where = 
             $this->wp->Criterion[1];
    }
//End Prepare Method

//Open Method @110-2D16873F
    function Open()
    {
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildSelect", $this->Parent);
        $this->SQL = "SELECT * \n\n" .
        "FROM V_SUBSCRIBER_CPE_RECU_TARIFF {SQL_Where} {SQL_OrderBy}";
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeExecuteSelect", $this->Parent);
        $this->query(CCBuildSQL($this->SQL, $this->Where, $this->Order));
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "AfterExecuteSelect", $this->Parent);
    }
...
//Insert Method @110-D67F776D
    function Insert()
    {
        ...
        $this->cp["SUBSCRIBER_CPE_TARIFF_ID"] = new clsSQLParameter("ctrlSUBSCRIBER_CPE_TARIFF_ID", ccsFloat, "", "", $this->SUBSCRIBER_CPE_TARIFF_ID->GetValue(true), "", false, $this->ErrorBlock);
        ...
        if (!is_null($this->cp["SUBSCRIBER_CPE_TARIFF_ID"]->GetValue()) and !strlen($this->cp["SUBSCRIBER_CPE_TARIFF_ID"]->GetText()) and !is_bool($this->cp["SUBSCRIBER_CPE_TARIFF_ID"]->GetValue())) 
            $this->cp["SUBSCRIBER_CPE_TARIFF_ID"]->SetValue($this->SUBSCRIBER_CPE_TARIFF_ID->GetValue(true));
        ...
        $this->SQL = "INSERT INTO SUBSCRIBER_CPE_RECU_TARIFF(\n" .
        ...
        " " . $this->SQLValue($this->cp["SUBSCRIBER_CPE_ID"]->GetDBValue(), ccsFloat) . ",\n" .
...
//Update Method @110-EF9E7F77
    function Update()
    {
        ...
        if (!is_null($this->cp["SUBSCRIBER_CPE_ID"]->GetValue()) and !strlen($this->cp["SUBSCRIBER_CPE_ID"]->GetText()) and !is_bool($this->cp["SUBSCRIBER_CPE_ID"]->GetValue())) 
            $this->cp["SUBSCRIBER_CPE_ID"]->SetValue($this->SUBSCRIBER_CPE_ID->GetValue(true));
        $this->SQL = "UPDATE V_SUBSCRIBER_CPE_RECU_TARIFF SET \n" .
        ...
        "UPDATE_DATE=SYSDATE\n" .
        "WHERE\n" .
        "SUBSCRIBER_CPE_TARIFF_ID=" . $this->SQLValue($this->cp["SUBSCRIBER_CPE_TARIFF_ID"]->GetDBValue(), ccsFloat) . "";
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeExecuteUpdate", $this->Parent);
        if($this->Errors->Count() == 0 && $this->CmdExecution) {
            $this->query($this->SQL);
            $this->CCSEventResult = CCGetEvent($this->CCSEvents, "AfterExecuteUpdate", $this->Parent);
...
//Delete Method @110-16BABEA4
    function Delete()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
        $this->CmdExecution = true;
        $this->cp["SUBSCRIBER_CPE_TARIFF_ID"] = new clsSQLParameter("ctrlSUBSCRIBER_CPE_TARIFF_ID", ccsFloat, "", "", $this->SUBSCRIBER_CPE_TARIFF_ID->GetValue(true), 0, false, $this->ErrorBlock);
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildDelete", $this->Parent);
        if (!is_null($this->cp["SUBSCRIBER_CPE_TARIFF_ID"]->GetValue()) and !strlen($this->cp["SUBSCRIBER_CPE_TARIFF_ID"]->GetText()) and !is_bool($this->cp["SUBSCRIBER_CPE_TARIFF_ID"]->GetValue())) 
            $this->cp["SUBSCRIBER_CPE_TARIFF_ID"]->SetValue($this->SUBSCRIBER_CPE_TARIFF_ID->GetValue(true));
        if (!strlen($this->cp["SUBSCRIBER_CPE_TARIFF_ID"]->GetText()) and !is_bool($this->cp["SUBSCRIBER_CPE_TARIFF_ID"]->GetValue(true))) 
            $this->cp["SUBSCRIBER_CPE_TARIFF_ID"]->SetText(0);
        $this->SQL = "DELETE FROM SUBSCRIBER_CPE_RECU_TARIFF WHERE  SUBSCRIBER_CPE_TARIFF_ID = " . $this->SQLValue($this->cp["SUBSCRIBER_CPE_TARIFF_ID"]->GetDBValue(), ccsFloat) . "";
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeExecuteDelete", $this->Parent);
        if($this->Errors->Count() == 0 && $this->CmdExecution) {
            $this->query($this->SQL);
            $this->CCSEventResult = CCGetEvent($this->CCSEvents, "AfterExecuteDelete", $this->Parent);
...
} //End CPE_RECURRING_TARIFFDataSource Class @110-FCB6E20C



//Initialize Page @1-AFD0FAC5
// Variables
$FileName = "";
$Redirect = "";
$Tpl = "";
...
$FileName = FileName;
$Redirect = "";
$TemplateFileName = "subscriber_cpe_form2.html";
$BlockToParse = "main";
...
//Include events file @1-C4F193A1
include_once("./subscriber_cpe_form2_events.php");
...
$CPE_RECURRING_TARIFF = & new clsRecordCPE_RECURRING_TARIFF("", $MainPage);
$MainPage->SubscribInfo = & $SubscribInfo;
$MainPage->CPE_RECURRING_TARIFF = & $CPE_RECURRING_TARIFF;
$SubscribInfo->Initialize();
$CPE_RECURRING_TARIFF->Initialize();
...
//Execute Components @1-E8B0CD0A
$SubscribInfo->Operation();
$CPE_RECURRING_TARIFF->Operation();
//End Execute Components

//Go to destination page @1-5DB6C2CA
if($Redirect)
{
    ...
    unset($SubscribInfo);
    unset($CPE_RECURRING_TARIFF);
    unset($Tpl);
    exit;
}
//End Go to destination page

//Show Page @1-F0E65344
$SubscribInfo->Show();
$CPE_RECURRING_TARIFF->Show();
$Tpl->block_path = "";
