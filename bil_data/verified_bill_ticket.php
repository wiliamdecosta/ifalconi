<?php
//Include Common Files @1-54D3C32D
define("RelativePath", "..");
define("PathToCurrentPage", "/bil_data/");
define("FileName", "verified_bill_ticket.php");
...
//Class_Initialize Event @2-B0CB6452
    function clsGridENTRY_BILL_TICKET($RelativePath, & $Parent)
    {
        ...
        $this->DLink->Page = "verified_bill_ticket.php";
        $this->ADLink = & new clsControl(ccsLink, "ADLink", "ADLink", ccsText, "", CCGetRequestParam("ADLink", ccsGet, NULL), $this);
        $this->ADLink->HTML = true;
        $this->ADLink->Page = "verified_bill_ticket.php";
...
//Open Method @2-C57F5F41
    function Open()
    {
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildSelect", $this->Parent);
        $this->CountSQL = "SELECT COUNT(*) FROM (SELECT *\n" .
        "FROM V_T_BILL_TICKET\n" .
        "WHERE VERIFICATION_DATE IS NOT NULL AND (UPPER(to_char(TICKET_DATE,'dd-MON-yyyy')) LIKE UPPER('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%') OR\n" .
...
//Operation Method @3-B42F52E3
    function Operation()
    {
        ...
        $Redirect = "verified_bill_ticket.php";
        if($this->Validate()) {
            if($this->PressedButton == "Button_DoSearch") {
                $Redirect = "verified_bill_ticket.php" . "?" . CCMergeQueryStrings(CCGetQueryString("Form", array("Button_DoSearch", "Button_DoSearch_x", "Button_DoSearch_y", "TAMBAH", "BATCH_CONTROL_ID")));
...
//Class_Initialize Event @19-002B4278
    function clsRecordBATCH1($RelativePath, & $Parent)
    {
        ...
        if($this->Visible)
        {
            ...
            $this->IS_OK = & new clsControl(ccsTextBox, "IS_OK", "IS_OK", ccsText, "", CCGetRequestParam("IS_OK", $Method, NULL), $this);
            ...
            $this->VERIFIED_BY = & new clsControl(ccsTextBox, "VERIFIED_BY", "VERIFIED_BY", ccsText, "", CCGetRequestParam("VERIFIED_BY", $Method, NULL), $this);
            $this->VERIFICATION_DATE = & new clsControl(ccsTextBox, "VERIFICATION_DATE", "VERIFICATION_DATE", ccsDate, array("dd", "-", "mmm", "-", "yyyy"), CCGetRequestParam("VERIFICATION_DATE", $Method, NULL), $this);
            if(!$this->FormSubmitted) {
                ...
                if(!is_array($this->VERIFIED_BY->Value) && !strlen($this->VERIFIED_BY->Value) && $this->VERIFIED_BY->Value !== false)
                    $this->VERIFIED_BY->SetText(CCGetUserLogin());
                if(!is_array($this->VERIFICATION_DATE->Value) && !strlen($this->VERIFICATION_DATE->Value) && $this->VERIFICATION_DATE->Value !== false)
                    $this->VERIFICATION_DATE->SetValue(time());
            }
        }
    }
//End Class_Initialize Event
...
//Validate Method @19-C10ACD98
    function Validate()
    {
        ...
        $Validation = ($this->VERIFIED_BY->Validate() && $Validation);
        $Validation = ($this->VERIFICATION_DATE->Validate() && $Validation);
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "OnValidate", $this);
        ...
        $Validation =  $Validation && ($this->VERIFIED_BY->Errors->Count() == 0);
        $Validation =  $Validation && ($this->VERIFICATION_DATE->Errors->Count() == 0);
        return (($this->Errors->Count() == 0) && $Validation);
    }
//End Validate Method

//CheckErrors Method @19-5FDF5EE5
    function CheckErrors()
    {
        $errors = false;
        ...
        $errors = ($errors || $this->VERIFICATION_DATE->Errors->Count());
        $errors = ($errors || $this->Errors->Count());
...
//Show Method @19-CCB3827C
    function Show()
    {
        ...
        if($this->EditMode) {
            ...
            if($this->DataSource->Errors->Count() == 0 && $this->DataSource->next_record()) {
                ...
                if(!$this->FormSubmitted){
                    ...
                    $this->VERIFICATION_DATE->SetValue($this->DataSource->VERIFICATION_DATE->GetValue());
                }
            } else {
                $this->EditMode = false;
            }
        ...
        if($this->FormSubmitted || $this->CheckErrors()) {
            ...
            $Error = ComposeStrings($Error, $this->VERIFICATION_DATE->Errors->ToString());
            $Error = ComposeStrings($Error, $this->Errors->ToString());
...
//DataSource Variables @19-787C0123
    var $Parent = "";
    var $CCSEvents = "";
    var $CCSEventResult;
    var $ErrorBlock;
    ...
    var $FINAL_AMOUNT;
    var $VERIFIED_BY;
    var $VERIFICATION_DATE;
//End DataSource Variables

//DataSourceClass_Initialize Event @19-9A8C890F
    function clsBATCH1DataSource(& $Parent)
    {
        ...
        $this->VERIFICATION_DATE = new clsField("VERIFICATION_DATE", ccsDate, $this->DateFormat);
...
//SetValues Method @19-08EDBFB3
    function SetValues()
    {
        ...
        $this->VERIFICATION_DATE->SetDBValue(trim($this->f("VERIFICATION_DATE")));
    }
//End SetValues Method
...
$FileName = FileName;
$Redirect = "";
$TemplateFileName = "verified_bill_ticket.html";
$BlockToParse = "main";
...
//Include events file @1-74D997D4
include_once("./verified_bill_ticket_events.php");
