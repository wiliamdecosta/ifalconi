...
class clsRecordV_SUBS_OT_PROMO_FEATURE { //V_SUBS_OT_PROMO_FEATURE Class @110-47E5CFAB
...
//Class_Initialize Event @110-2280C86E
    function clsRecordV_SUBS_OT_PROMO_FEATURE($RelativePath, & $Parent)
    {
        ...
        if($this->Visible)
        {
            ...
            $this->P_FEATURE_PROMOTION_ID = & new clsControl(ccsHidden, "P_FEATURE_PROMOTION_ID", "P ONETIME PROMOTION ID", ccsFloat, "", CCGetRequestParam("P_FEATURE_PROMOTION_ID", $Method, NULL), $this);
            $this->P_FEATURE_PROMOTION_ID->Required = true;
            $this->PROMOTION_DATE = & new clsControl(ccsTextBox, "PROMOTION_DATE", "PROMOTION DATE", ccsDate, array("dd", "-", "mmm", "-", "yyyy"), CCGetRequestParam("PROMOTION_DATE", $Method, NULL), $this);
...
//Initialize Method @110-BEAFE27B
    function Initialize()
    {
...
//CheckErrors Method @110-F610967D
...
//Operation Method @110-8B5C0083
    function Operation()
    {
        ...
        if($this->PressedButton == "Button_Delete") {
            ...
            $Redirect = "subscriber_promo.php" . "?" . CCGetQueryString("QueryString", array("ccsForm", "SUBS_OT_PROMO_SERVICE_ID"));
...
} //End V_SUBS_OT_PROMO_FEATURE Class @110-FCB6E20C

class clsV_SUBS_OT_PROMO_FEATUREDataSource extends clsDBConn {  //V_SUBS_OT_PROMO_FEATUREDataSource Class @110-682E7C2B

//DataSource Variables @110-69CEE610
    var $Parent = "";
    var $CCSEvents = "";
    var $CCSEventResult;
    var $ErrorBlock;
...
//DataSourceClass_Initialize Event @110-346FD8C2
...
//Update Method @110-B3B7F227
    function Update()
    {
        ...
        if (!is_null($this->cp["UPDATE_BY"]->GetValue()) and !strlen($this->cp["UPDATE_BY"]->GetText()) and !is_bool($this->cp["UPDATE_BY"]->GetValue())) 
            $this->cp["UPDATE_BY"]->SetValue(CCGetSession("UserName", NULL));
        $this->SQL = "UPDATE SUBS_OT_PROMO_FEATURE SET \n" .
        "P_FEATURE_PROMOTION_ID=" . $this->SQLValue($this->cp["P_FEATURE_PROMOTION_ID"]->GetDBValue(), ccsFloat) . ",\n" .
...
//Delete Method @110-0217234D
    function Delete()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
        $this->CmdExecution = true;
        $this->cp["SUBS_OT_PROMO_FEATURE_ID"] = new clsSQLParameter("ctrlSUBS_OT_PROMO_FEATURE_ID", ccsFloat, "", "", $this->SUBS_OT_PROMO_FEATURE_ID->GetValue(true), 0, false, $this->ErrorBlock);
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildDelete", $this->Parent);
        if (!is_null($this->cp["SUBS_OT_PROMO_FEATURE_ID"]->GetValue()) and !strlen($this->cp["SUBS_OT_PROMO_FEATURE_ID"]->GetText()) and !is_bool($this->cp["SUBS_OT_PROMO_FEATURE_ID"]->GetValue())) 
            $this->cp["SUBS_OT_PROMO_FEATURE_ID"]->SetValue($this->SUBS_OT_PROMO_FEATURE_ID->GetValue(true));
        if (!strlen($this->cp["SUBS_OT_PROMO_FEATURE_ID"]->GetText()) and !is_bool($this->cp["SUBS_OT_PROMO_FEATURE_ID"]->GetValue(true))) 
            $this->cp["SUBS_OT_PROMO_FEATURE_ID"]->SetText(0);
        $this->SQL = "DELETE FROM SUBS_OT_PROMO_FEATURE WHERE SUBS_OT_PROMO_FEATURE_ID = " . $this->SQLValue($this->cp["SUBS_OT_PROMO_FEATURE_ID"]->GetDBValue(), ccsFloat) . "";
...
} //End V_SUBS_OT_PROMO_FEATUREDataSource Class @110-FCB6E20C
...
//Execute Components @1-1A9D1D4B
$V_SUBS_OT_PROMO_FEATURE->Operation();
$SubscribInfo->Operation();
//End Execute Components

//Go to destination page @1-CCD48CB1
if($Redirect)
{
