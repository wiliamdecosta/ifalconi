<?php
//Include Common Files @1-1264AB61
define("RelativePath", "..");
define("PathToCurrentPage", "/bil_data/");
define("FileName", "subscriber_tariff_override_form1.php");
...
//Class_Initialize Event @110-22CF8B35
    function clsRecordV_SUBS_OVERRIDE_RECU_TARI($RelativePath, & $Parent)
    {
        ...
        if($this->Visible)
        {
            ...
            $this->OT_PROMOTION_CODE = & new clsControl(ccsTextBox, "OT_PROMOTION_CODE", "OT PROMOTION CODE", ccsText, "", CCGetRequestParam("OT_PROMOTION_CODE", $Method, NULL), $this);
            ...
            $this->SUBS_OVERRIDE_RECU_TARIFF_ID = & new clsControl(ccsHidden, "SUBS_OVERRIDE_RECU_TARIFF_ID", "SUBS_OVERRIDE_RECU_TARIFF_ID", ccsFloat, "", CCGetRequestParam("SUBS_OVERRIDE_RECU_TARIFF_ID", $Method, NULL), $this);
            if(!$this->FormSubmitted) {
...
//Initialize Method @110-54F40960
    function Initialize()
    {
        ...
        $this->DataSource->Parameters["urlSUBS_OVERRIDE_RECU_TARIFF_ID"] = CCGetFromGet("SUBS_OVERRIDE_RECU_TARIFF_ID", NULL);
    }
//End Initialize Method

//Validate Method @110-BEB19A66
    function Validate()
    {
        global $CCSLocales;
        $Validation = true;
        ...
        $Validation = ($this->OT_PROMOTION_CODE->Validate() && $Validation);
        $Validation = ($this->UPDATE_DATE->Validate() && $Validation);
        $Validation = ($this->SUBS_OVERRIDE_RECU_TARIFF_ID->Validate() && $Validation);
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "OnValidate", $this);
        ...
        $Validation =  $Validation && ($this->OT_PROMOTION_CODE->Errors->Count() == 0);
        $Validation =  $Validation && ($this->UPDATE_DATE->Errors->Count() == 0);
        $Validation =  $Validation && ($this->SUBS_OVERRIDE_RECU_TARIFF_ID->Errors->Count() == 0);
        return (($this->Errors->Count() == 0) && $Validation);
    }
//End Validate Method

//CheckErrors Method @110-28565AEA
    function CheckErrors()
    {
        $errors = false;
        ...
        $errors = ($errors || $this->OT_PROMOTION_CODE->Errors->Count());
        $errors = ($errors || $this->UPDATE_DATE->Errors->Count());
        $errors = ($errors || $this->SUBS_OVERRIDE_RECU_TARIFF_ID->Errors->Count());
        $errors = ($errors || $this->Errors->Count());
        $errors = ($errors || $this->DataSource->Errors->Count());
        return $errors;
    }
...
//Operation Method @110-C2C349A8
    function Operation()
    {
        ...
        if($this->PressedButton == "Button_Delete") {
            $Redirect = "subscriber_tariff_override.php" . "?" . CCGetQueryString("QueryString", array("ccsForm", "SUBS_OVERRIDE_RECU_TARIFF_ID"));
...
//InsertRow Method @110-E3AE28F7
    function InsertRow()
    {
        ...
        $this->DataSource->OT_PROMOTION_CODE->SetValue($this->OT_PROMOTION_CODE->GetValue(true));
        $this->DataSource->SUBS_OVERRIDE_RECU_TARIFF_ID->SetValue($this->SUBS_OVERRIDE_RECU_TARIFF_ID->GetValue(true));
        $this->DataSource->Insert();
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "AfterInsert", $this);
        return (!$this->CheckErrors());
    }
...
//Show Method @110-6D348819
    function Show()
    {
        ...
        if($this->EditMode) {
            ...
            if($this->DataSource->Errors->Count() == 0 && $this->DataSource->next_record()) {
                ...
                if(!$this->FormSubmitted){
                    ...
                    $this->SUBS_OVERRIDE_RECU_TARIFF_ID->SetValue($this->DataSource->SUBS_OVERRIDE_RECU_TARIFF_ID->GetValue());
                }
            } else {
                $this->EditMode = false;
            }
        ...
        if($this->FormSubmitted || $this->CheckErrors()) {
            ...
            $Error = ComposeStrings($Error, $this->OT_PROMOTION_CODE->Errors->ToString());
            $Error = ComposeStrings($Error, $this->UPDATE_DATE->Errors->ToString());
            $Error = ComposeStrings($Error, $this->SUBS_OVERRIDE_RECU_TARIFF_ID->Errors->ToString());
            $Error = ComposeStrings($Error, $this->Errors->ToString());
            $Error = ComposeStrings($Error, $this->DataSource->Errors->ToString());
            $Tpl->SetVar("Error", $Error);
            $Tpl->Parse("Error", false);
        ...
        $this->SUBS_OVERRIDE_RECU_TARIFF_ID->Show();
        $Tpl->parse();
        $Tpl->block_path = $ParentPath;
        $this->DataSource->close();
    }
...
//DataSource Variables @110-9D5625A0
    var $Parent = "";
    var $CCSEvents = "";
    var $CCSEventResult;
    var $ErrorBlock;
    ...
    var $UPDATE_DATE;
    var $SUBS_OVERRIDE_RECU_TARIFF_ID;
//End DataSource Variables

//DataSourceClass_Initialize Event @110-DDBE7A85
    function clsV_SUBS_OVERRIDE_RECU_TARIDataSource(& $Parent)
    {
        ...
        $this->SUBS_OVERRIDE_RECU_TARIFF_ID = new clsField("SUBS_OVERRIDE_RECU_TARIFF_ID", ccsFloat, "");
        

    }
//End DataSourceClass_Initialize Event

//Prepare Method @110-DEAD732E
    function Prepare()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
        $this->wp = new clsSQLParameters($this->ErrorBlock);
        $this->wp->AddParameter("1", "urlSUBS_OVERRIDE_RECU_TARIFF_ID", ccsFloat, "", "", $this->Parameters["urlSUBS_OVERRIDE_RECU_TARIFF_ID"], "", false);
        $this->AllParametersSet = $this->wp->AllParamsSet();
        $this->wp->Criterion[1] = $this->wp->Operation(opEqual, "SUBS_OVERRIDE_RECU_TARIFF_ID", $this->wp->GetDBValue("1"), $this->ToSQL($this->wp->GetDBValue("1"), ccsFloat),false);
        $this->Where = 
             $this->wp->Criterion[1];
    }
//End Prepare Method

//Open Method @110-388BD4C5
    function Open()
    {
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildSelect", $this->Parent);
        $this->SQL = "SELECT * \n\n" .
        "FROM V_SUBS_OVERRIDE_RECU_TARIFF {SQL_Where} {SQL_OrderBy}";
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeExecuteSelect", $this->Parent);
        $this->query(CCBuildSQL($this->SQL, $this->Where, $this->Order));
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "AfterExecuteSelect", $this->Parent);
    }
...
//SetValues Method @110-DF441E38
    function SetValues()
    {
        ...
        $this->SUBS_OT_PROMO_SERVICE_ID->SetDBValue(trim($this->f("SUBS_OT_PROMO_SERVICE_ID")));
        ...
        $this->SUBS_OVERRIDE_RECU_TARIFF_ID->SetDBValue(trim($this->f("SUBS_OVERRIDE_RECU_TARIFF_ID")));
    }
//End SetValues Method

//Insert Method @110-CAD92264
    function Insert()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
        ...
        $this->cp["OT_PROMOTION_CODE"] = new clsSQLParameter("ctrlOT_PROMOTION_CODE", ccsText, "", "", $this->OT_PROMOTION_CODE->GetValue(true), "", false, $this->ErrorBlock);
        $this->cp["SUBS_OVERRIDE_RECU_TARIFF_ID"] = new clsSQLParameter("ctrlSUBS_OVERRIDE_RECU_TARIFF_ID", ccsFloat, "", "", $this->SUBS_OVERRIDE_RECU_TARIFF_ID->GetValue(true), "", false, $this->ErrorBlock);
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildInsert", $this->Parent);
        ...
        if (!is_null($this->cp["OT_PROMOTION_CODE"]->GetValue()) and !strlen($this->cp["OT_PROMOTION_CODE"]->GetText()) and !is_bool($this->cp["OT_PROMOTION_CODE"]->GetValue())) 
            $this->cp["OT_PROMOTION_CODE"]->SetValue($this->OT_PROMOTION_CODE->GetValue(true));
        if (!is_null($this->cp["SUBS_OVERRIDE_RECU_TARIFF_ID"]->GetValue()) and !strlen($this->cp["SUBS_OVERRIDE_RECU_TARIFF_ID"]->GetText()) and !is_bool($this->cp["SUBS_OVERRIDE_RECU_TARIFF_ID"]->GetValue())) 
            $this->cp["SUBS_OVERRIDE_RECU_TARIFF_ID"]->SetValue($this->SUBS_OVERRIDE_RECU_TARIFF_ID->GetValue(true));
        $this->SQL = "INSERT INTO SUBS_OVERRIDE_RECU_TARIFF(\n" .
...
//Update Method @110-ABD5C952
    function Update()
    {
        ...
        $this->cp["DESCRIPTION"] = new clsSQLParameter("ctrlDESCRIPTION", ccsText, "", "", $this->DESCRIPTION->GetValue(true), "", false, $this->ErrorBlock);
        $this->cp["VALID_TO"] = new clsSQLParameter("ctrlVALID_TO", ccsDate, array("dd", "-", "mmm", "-", "yyyy"), $this->DateFormat, $this->VALID_TO->GetValue(true), "", false, $this->ErrorBlock);
        $this->cp["SUBS_OVERRIDE_RECU_TARIFF_ID"] = new clsSQLParameter("ctrlSUBS_OVERRIDE_RECU_TARIFF_ID", ccsFloat, "", "", $this->SUBS_OVERRIDE_RECU_TARIFF_ID->GetValue(true), 0, false, $this->ErrorBlock);
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildUpdate", $this->Parent);
        if (!is_null($this->cp["P_BILL_COMPONENT_ID"]->GetValue()) and !strlen($this->cp["P_BILL_COMPONENT_ID"]->GetText()) and !is_bool($this->cp["P_BILL_COMPONENT_ID"]->GetValue())) 
            $this->cp["P_BILL_COMPONENT_ID"]->SetValue($this->P_BILL_COMPONENT_ID->GetValue(true));
        if (!is_null($this->cp["VALID_FROM"]->GetValue()) and !strlen($this->cp["VALID_FROM"]->GetText()) and !is_bool($this->cp["VALID_FROM"]->GetValue())) 
        ...
        if (!is_null($this->cp["SUBS_OVERRIDE_RECU_TARIFF_ID"]->GetValue()) and !strlen($this->cp["SUBS_OVERRIDE_RECU_TARIFF_ID"]->GetText()) and !is_bool($this->cp["SUBS_OVERRIDE_RECU_TARIFF_ID"]->GetValue())) 
            $this->cp["SUBS_OVERRIDE_RECU_TARIFF_ID"]->SetValue($this->SUBS_OVERRIDE_RECU_TARIFF_ID->GetValue(true));
        if (!strlen($this->cp["SUBS_OVERRIDE_RECU_TARIFF_ID"]->GetText()) and !is_bool($this->cp["SUBS_OVERRIDE_RECU_TARIFF_ID"]->GetValue(true))) 
            $this->cp["SUBS_OVERRIDE_RECU_TARIFF_ID"]->SetText(0);
        $this->SQL = "UPDATE SUBS_OVERRIDE_RECU_TARIFF SET  \n" .
        ...
        "UPDATE_DATE=SYSDATE\n" .
        "WHERE  SUBS_OVERRIDE_RECU_TARIFF_ID = " . $this->SQLValue($this->cp["SUBS_OVERRIDE_RECU_TARIFF_ID"]->GetDBValue(), ccsFloat) . "";
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeExecuteUpdate", $this->Parent);
        if($this->Errors->Count() == 0 && $this->CmdExecution) {
            $this->query($this->SQL);
            $this->CCSEventResult = CCGetEvent($this->CCSEvents, "AfterExecuteUpdate", $this->Parent);
...
//Delete Method @110-4ED26554
    function Delete()
    {
        ...
        if (!strlen($this->cp["SUBS_OVERRIDE_RECU_TARIFF_ID"]->GetText()) and !is_bool($this->cp["SUBS_OVERRIDE_RECU_TARIFF_ID"]->GetValue(true))) 
            $this->cp["SUBS_OVERRIDE_RECU_TARIFF_ID"]->SetText(0);
        $this->SQL = "DELETE FROM SUBS_OVERRIDE_RECU_TARIFF WHERE  SUBS_OVERRIDE_RECU_TARIFF_ID = " . $this->SQLValue($this->cp["SUBS_OVERRIDE_RECU_TARIFF_ID"]->GetDBValue(), ccsFloat) . "";
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeExecuteDelete", $this->Parent);
        if($this->Errors->Count() == 0 && $this->CmdExecution) {
            $this->query($this->SQL);
            $this->CCSEventResult = CCGetEvent($this->CCSEvents, "AfterExecuteDelete", $this->Parent);
...
$FileName = FileName;
$Redirect = "";
$TemplateFileName = "subscriber_tariff_override_form1.html";
$BlockToParse = "main";
...
//Include events file @1-F026A893
include_once("./subscriber_tariff_override_form1_events.php");
