<?php
//Include Common Files @1-3964C693
define("RelativePath", "..");
define("PathToCurrentPage", "/bil_data/");
define("FileName", "subscriber_tariff_override_form3.php");
...
//Class_Initialize Event @110-A79C5B5A
    function clsRecordV_SUBS_OVERRIDE_RECU_TARI($RelativePath, & $Parent)
    {
        ...
        if($this->Visible)
        {
            ...
            $this->P_BUNDLED_FEATURE_ID = & new clsControl(ccsHidden, "P_BUNDLED_FEATURE_ID", "P BUNDLED FEATURE ID", ccsFloat, "", CCGetRequestParam("P_BUNDLED_FEATURE_ID", $Method, NULL), $this);
            ...
            $this->SUBS_OVER_BF_RECU_TARIFF_ID = & new clsControl(ccsHidden, "SUBS_OVER_BF_RECU_TARIFF_ID", "SUBS_OVER_BF_RECU_TARIFF_ID", ccsFloat, "", CCGetRequestParam("SUBS_OVER_BF_RECU_TARIFF_ID", $Method, NULL), $this);
            if(!$this->FormSubmitted) {
...
//Initialize Method @110-AFD205D8
    function Initialize()
    {
        ...
        $this->DataSource->Parameters["urlSUBS_OVER_BF_RECU_TARIFF_ID"] = CCGetFromGet("SUBS_OVER_BF_RECU_TARIFF_ID", NULL);
    }
//End Initialize Method

//Validate Method @110-70E29721
    function Validate()
    {
        global $CCSLocales;
        $Validation = true;
        ...
        $Validation = ($this->SUBS_OVER_BF_RECU_TARIFF_ID->Validate() && $Validation);
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "OnValidate", $this);
        ...
        $Validation =  $Validation && ($this->SUBS_OVER_BF_RECU_TARIFF_ID->Errors->Count() == 0);
        return (($this->Errors->Count() == 0) && $Validation);
    }
//End Validate Method

//CheckErrors Method @110-B0B31A74
    function CheckErrors()
    {
        $errors = false;
        ...
        $errors = ($errors || $this->SUBS_OVER_BF_RECU_TARIFF_ID->Errors->Count());
        $errors = ($errors || $this->Errors->Count());
        $errors = ($errors || $this->DataSource->Errors->Count());
        return $errors;
    }
...
//Operation Method @110-73C407BD
    function Operation()
    {
        ...
        if($this->PressedButton == "Button_Delete") {
            $Redirect = "subscriber_tariff_override.php" . "?" . CCGetQueryString("QueryString", array("ccsForm", "SUBS_OVER_BF_RECU_TARIFF_ID"));
...
//UpdateRow Method @110-121FF5B8
    function UpdateRow()
    {
        ...
        $this->DataSource->VALID_TO->SetValue($this->VALID_TO->GetValue(true));
        $this->DataSource->P_BILL_COMPONENT_ID->SetValue($this->P_BILL_COMPONENT_ID->GetValue(true));
        $this->DataSource->SUBS_OVER_BF_RECU_TARIFF_ID->SetValue($this->SUBS_OVER_BF_RECU_TARIFF_ID->GetValue(true));
        $this->DataSource->Update();
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "AfterUpdate", $this);
        return (!$this->CheckErrors());
    }
...
//Show Method @110-0522FE03
    function Show()
    {
        ...
        if($this->FormSubmitted || $this->CheckErrors()) {
            ...
            $Error = ComposeStrings($Error, $this->SUBS_OVER_BF_RECU_TARIFF_ID->Errors->ToString());
            $Error = ComposeStrings($Error, $this->Errors->ToString());
            $Error = ComposeStrings($Error, $this->DataSource->Errors->ToString());
            $Tpl->SetVar("Error", $Error);
            $Tpl->Parse("Error", false);
...
//DataSource Variables @110-66627333
    var $Parent = "";
    var $CCSEvents = "";
    var $CCSEventResult;
    var $ErrorBlock;
...
//DataSourceClass_Initialize Event @110-B3153965
    function clsV_SUBS_OVERRIDE_RECU_TARIDataSource(& $Parent)
    {
        ...
        $this->SUBS_OVER_BF_RECU_TARIFF_ID = new clsField("SUBS_OVER_BF_RECU_TARIFF_ID", ccsFloat, "");
        

    }
//End DataSourceClass_Initialize Event

//Prepare Method @110-276D003B
    function Prepare()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
        $this->wp = new clsSQLParameters($this->ErrorBlock);
        $this->wp->AddParameter("1", "urlSUBS_OVER_BF_RECU_TARIFF_ID", ccsFloat, "", "", $this->Parameters["urlSUBS_OVER_BF_RECU_TARIFF_ID"], "", false);
        $this->AllParametersSet = $this->wp->AllParamsSet();
        $this->wp->Criterion[1] = $this->wp->Operation(opEqual, "SUBS_OVER_BF_RECU_TARIFF_ID", $this->wp->GetDBValue("1"), $this->ToSQL($this->wp->GetDBValue("1"), ccsFloat),false);
        $this->Where = 
             $this->wp->Criterion[1];
    }
//End Prepare Method

//Open Method @110-44C7E6B3
    function Open()
    {
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildSelect", $this->Parent);
        $this->SQL = "SELECT * \n\n" .
        "FROM V_SUBS_OVER_BF_RECU_TARIFF {SQL_Where} {SQL_OrderBy}";
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeExecuteSelect", $this->Parent);
        $this->query(CCBuildSQL($this->SQL, $this->Where, $this->Order));
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "AfterExecuteSelect", $this->Parent);
    }
...
//SetValues Method @110-6AC28702
    function SetValues()
    {
        ...
        $this->SUBS_OVER_BF_RECU_TARIFF_ID->SetDBValue(trim($this->f("SUBS_OVER_BF_RECU_TARIFF_ID")));
    }
//End SetValues Method

//Insert Method @110-E77676A5
    function Insert()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
        ...
        $this->SQL = "INSERT INTO SUBS_OVER_BF_RECU_TARIFF(\n" .
...
//Update Method @110-C375E5EE
    function Update()
    {
        ...
        $this->cp["VALID_TO"] = new clsSQLParameter("ctrlVALID_TO", ccsDate, array("dd", "-", "mmm", "-", "yyyy"), $this->DateFormat, $this->VALID_TO->GetValue(true), "", false, $this->ErrorBlock);
        $this->cp["P_BILL_COMPONENT_ID"] = new clsSQLParameter("ctrlP_BILL_COMPONENT_ID", ccsFloat, "", "", $this->P_BILL_COMPONENT_ID->GetValue(true), "", false, $this->ErrorBlock);
        $this->cp["SUBS_OVER_BF_RECU_TARIFF_ID"] = new clsSQLParameter("ctrlSUBS_OVER_BF_RECU_TARIFF_ID", ccsFloat, "", "", $this->SUBS_OVER_BF_RECU_TARIFF_ID->GetValue(true), "", false, $this->ErrorBlock);
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildUpdate", $this->Parent);
        if (!is_null($this->cp["SUBSCRIBER_ID"]->GetValue()) and !strlen($this->cp["SUBSCRIBER_ID"]->GetText()) and !is_bool($this->cp["SUBSCRIBER_ID"]->GetValue())) 
            $this->cp["SUBSCRIBER_ID"]->SetValue($this->SUBSCRIBER_ID->GetValue(true));
        if (!is_null($this->cp["P_BUNDLED_FEATURE_ID"]->GetValue()) and !strlen($this->cp["P_BUNDLED_FEATURE_ID"]->GetText()) and !is_bool($this->cp["P_BUNDLED_FEATURE_ID"]->GetValue())) 
        ...
        if (!is_null($this->cp["P_BILL_COMPONENT_ID"]->GetValue()) and !strlen($this->cp["P_BILL_COMPONENT_ID"]->GetText()) and !is_bool($this->cp["P_BILL_COMPONENT_ID"]->GetValue())) 
            $this->cp["P_BILL_COMPONENT_ID"]->SetValue($this->P_BILL_COMPONENT_ID->GetValue(true));
        if (!is_null($this->cp["SUBS_OVER_BF_RECU_TARIFF_ID"]->GetValue()) and !strlen($this->cp["SUBS_OVER_BF_RECU_TARIFF_ID"]->GetText()) and !is_bool($this->cp["SUBS_OVER_BF_RECU_TARIFF_ID"]->GetValue())) 
            $this->cp["SUBS_OVER_BF_RECU_TARIFF_ID"]->SetValue($this->SUBS_OVER_BF_RECU_TARIFF_ID->GetValue(true));
        $this->SQL = "UPDATE SUBS_OVER_BF_RECU_TARIFF SET \n" .
        ...
        "WHERE\n" .
        "SUBS_OVER_BF_RECU_TARIFF_ID=" . $this->SQLValue($this->cp["SUBS_OVER_BF_RECU_TARIFF_ID"]->GetDBValue(), ccsFloat) . "";
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeExecuteUpdate", $this->Parent);
        if($this->Errors->Count() == 0 && $this->CmdExecution) {
            $this->query($this->SQL);
            $this->CCSEventResult = CCGetEvent($this->CCSEvents, "AfterExecuteUpdate", $this->Parent);
...
//Delete Method @110-ADA28E07
    function Delete()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
        $this->CmdExecution = true;
        $this->cp["SUBS_OVER_BF_RECU_TARIFF_ID"] = new clsSQLParameter("ctrlSUBS_OVER_BF_RECU_TARIFF_ID", ccsFloat, "", "", $this->SUBS_OVER_BF_RECU_TARIFF_ID->GetValue(true), 0, false, $this->ErrorBlock);
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildDelete", $this->Parent);
        if (!is_null($this->cp["SUBS_OVER_BF_RECU_TARIFF_ID"]->GetValue()) and !strlen($this->cp["SUBS_OVER_BF_RECU_TARIFF_ID"]->GetText()) and !is_bool($this->cp["SUBS_OVER_BF_RECU_TARIFF_ID"]->GetValue())) 
            $this->cp["SUBS_OVER_BF_RECU_TARIFF_ID"]->SetValue($this->SUBS_OVER_BF_RECU_TARIFF_ID->GetValue(true));
        if (!strlen($this->cp["SUBS_OVER_BF_RECU_TARIFF_ID"]->GetText()) and !is_bool($this->cp["SUBS_OVER_BF_RECU_TARIFF_ID"]->GetValue(true))) 
            $this->cp["SUBS_OVER_BF_RECU_TARIFF_ID"]->SetText(0);
        $this->SQL = "DELETE FROM SUBS_OVER_BF_RECU_TARIFF WHERE  SUBS_OVER_BF_RECU_TARIFF_ID = " . $this->SQLValue($this->cp["SUBS_OVER_BF_RECU_TARIFF_ID"]->GetDBValue(), ccsFloat) . "";
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeExecuteDelete", $this->Parent);
        if($this->Errors->Count() == 0 && $this->CmdExecution) {
            $this->query($this->SQL);
            $this->CCSEventResult = CCGetEvent($this->CCSEvents, "AfterExecuteDelete", $this->Parent);
...
$FileName = FileName;
$Redirect = "";
$TemplateFileName = "subscriber_tariff_override_form3.html";
$BlockToParse = "main";
...
//Include events file @1-58A01902
include_once("./subscriber_tariff_override_form3_events.php");
