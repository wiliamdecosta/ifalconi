<?php
//Include Common Files @1-C3934770
define("RelativePath", "..");
...
//Open Method @2-7E374F2B
    function Open()
    {
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildSelect", $this->Parent);
        $this->CountSQL = "SELECT COUNT(*) FROM (select a.*,b.code as p_job_type_name from p_job a , p_job_type b\n" .
...
//Class_Initialize Event @31-84FF1764
...
//CheckErrors Method @31-CD58736A
    function CheckErrors()
    {
        $errors = false;
...
//DataSource Variables @31-C0F886DC
    var $Parent = "";
    var $CCSEvents = "";
    var $CCSEventResult;
    var $ErrorBlock;
...
//DataSourceClass_Initialize Event @31-6FA9DF60
...
//Insert Method @31-90FCE0C8
    function Insert()
    {
        ...
        $this->cp["CANCEL_PARENT_ID"] = new clsSQLParameter("ctrlCANCEL_PARENT_ID", ccsFloat, "", "", $this->CANCEL_PARENT_ID->GetValue(true), 0, false, $this->ErrorBlock);
        ...
        if (!strlen($this->cp["CANCEL_PARENT_ID"]->GetText()) and !is_bool($this->cp["CANCEL_PARENT_ID"]->GetValue(true))) 
            $this->cp["CANCEL_PARENT_ID"]->SetText(0);
        ...
        $this->SQL = "INSERT INTO P_JOB(P_JOB_ID, CODE, PROCEDURE_NAME, LISTING_NO, IS_PARALLEL, PARALLEL_DEGREE, IS_FINISH, P_JOB_TYPE_ID, IS_CANCELLED_PROCESS, IS_REPROCESS, PARENT_ID, CANCEL_PARENT_ID, CONTROL_TABLE_NAME, DESCRIPTION, CREATION_DATE, CREATED_BY, UPDATED_DATE, UPDATED_BY) \n" .
        "VALUES\n" .
        "(GENERATE_ID('TRB','P_JOB','P_JOB_ID'),'" . $this->SQLValue($this->cp["CODE"]->GetDBValue(), ccsText) . "','" . $this->SQLValue($this->cp["PROCEDURE_NAME"]->GetDBValue(), ccsText) . "'," . $this->SQLValue($this->cp["LISTING_NO"]->GetDBValue(), ccsFloat) . ",'" . $this->SQLValue($this->cp["IS_PARALLEL"]->GetDBValue(), ccsText) . "'," . $this->SQLValue($this->cp["PARALLEL_DEGREE"]->GetDBValue(), ccsFloat) . ",'" . $this->SQLValue($this->cp["IS_FINISH"]->GetDBValue(), ccsText) . "'," . $this->SQLValue($this->cp["P_JOB_TYPE_ID"]->GetDBValue(), ccsFloat) . ",'" . $this->SQLValue($this->cp["IS_CANCELLED_PROCESS"]->GetDBValue(), ccsText) . "','" . $this->SQLValue($this->cp["IS_REPROCESS"]->GetDBValue(), ccsText) . "'," . $this->SQLValue($this->cp["PARENT_ID"]->GetDBValue(), ccsFloat) . "," . $this->SQLValue($this->cp["CANCEL_PARENT_ID"]->GetDBValue(), ccsFloat) . ",'" . $this->SQLValue($this->cp["CONTROL_TABLE_NAME"]->GetDBValue(), ccsText) . "','" . $this->SQLValue($this->cp["DESCRIPTION"]->GetDBValue(), ccsText) . "', sysdate, '" . $this->SQLValue($this->cp["CREATED_BY"]->GetDBValue(), ccsText) . "', sysdate, '" . $this->SQLValue($this->cp["UPDATED_BY"]->GetDBValue(), ccsText) . "') ";
