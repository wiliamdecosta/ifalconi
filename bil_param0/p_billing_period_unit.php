<?php
//Include Common Files @1-DDFAC670
define("RelativePath", "..");
...
//Class_Initialize Event @2-4575BE14
    function clsGridP_BILLING_PERIOD_UNIT($RelativePath, & $Parent)
    {
        ...
        $this->ADLink->Page = "p_billing_period_unit.php";
    ...
    function Show()
    {
        ...
        if (!$this->IsEmpty) {
            ...
            while ($this->ForceIteration || (($this->RowNumber < $this->PageSize) &&  ($this->HasRecord = $this->DataSource->has_next_record()))) {
                ...
                $this->ADLink->Parameters = CCAddParam($this->ADLink->Parameters, "P_BILLING_PERIOD_UNIT_ID", $this->DataSource->f("P_BILLING_PERIOD_UNIT_ID"));
        ...
        $this->P_BILLING_PERIOD_UNIT_Insert->Parameters = CCAddParam($this->P_BILLING_PERIOD_UNIT_Insert->Parameters, "FLAG", "ADD");
        $this->Navigator->Show();
...
//DataSource Variables @2-87679365
    var $Parent = "";
    var $CCSEvents = "";
    var $CCSEventResult;
    var $ErrorBlock;
...
//DataSourceClass_Initialize Event @2-6D0A65A5
...
//SetValues Method @2-67497E1F
    function SetValues()
    {
        $this->CODE->SetDBValue($this->f("CODE"));
        $this->DAY_AMT->SetDBValue(trim($this->f("DAY_AMT")));
        $this->DESCRIPTION->SetDBValue($this->f("DESCRIPTION"));
        $this->DLink->SetDBValue($this->f("P_BILLING_PERIOD_UNIT_ID"));
        $this->ADLink->SetDBValue($this->f("P_BILLING_PERIOD_UNIT_ID"));
    ...
    function Operation()
    {
        ...
        if($this->Validate()) {
            if($this->PressedButton == "Button_DoSearch") {
                $Redirect = "p_billing_period_unit.php" . "?" . CCMergeQueryStrings(CCGetQueryString("Form", array("Button_DoSearch", "Button_DoSearch_x", "Button_DoSearch_y")));
...
class clsRecordp_billing_period_unit1 { //p_billing_period_unit1 Class @22-3D940671
...
//Class_Initialize Event @22-7F0A383A
...
//CheckErrors Method @22-0F135D5E
    function CheckErrors()
    {
        $errors = false;
...
} //End p_billing_period_unit1 Class @22-FCB6E20C

class clsp_billing_period_unit1DataSource extends clsDBConn {  //p_billing_period_unit1DataSource Class @22-CAD89780

//DataSource Variables @22-596214C3
    var $Parent = "";
    var $CCSEvents = "";
    var $CCSEventResult;
    var $ErrorBlock;
...
//DataSourceClass_Initialize Event @22-10A9E911
...
//Insert Method @22-B722CC01
    function Insert()
    {
        ...
        $this->SQL = "INSERT INTO P_BILLING_PERIOD_UNIT(P_BILLING_PERIOD_UNIT_ID, CODE, DAY_AMT, DESCRIPTION, CREATION_DATE, CREATED_BY, UPDATED_DATE, UPDATED_BY)\n" .
        "VALUES\n" .
        "(GENERATE_ID('TRB','P_BILLING_PERIOD_UNIT','P_BILLING_PERIOD_UNIT_ID'),UPPER(TRIM('" . $this->SQLValue($this->cp["CODE"]->GetDBValue(), ccsText) . "'))," . $this->SQLValue($this->cp["DAY_AMT"]->GetDBValue(), ccsFloat) . ",'" . $this->SQLValue($this->cp["DESCRIPTION"]->GetDBValue(), ccsText) . "',sysdate, '" . $this->SQLValue($this->cp["CREATED_BY"]->GetDBValue(), ccsText) . "',sysdate, '" . $this->SQLValue($this->cp["UPDATED_BY"]->GetDBValue(), ccsText) . "')";
...
//Update Method @22-26BA3D5E
    function Update()
    {
        ...
        $this->SQL = "UPDATE P_BILLING_PERIOD_UNIT SET \n" .
...
//Delete Method @22-24DBAFB3
    function Delete()
    {
        ...
        if (!strlen($this->cp["P_BILLING_PERIOD_UNIT_ID"]->GetText()) and !is_bool($this->cp["P_BILLING_PERIOD_UNIT_ID"]->GetValue(true))) 
            $this->cp["P_BILLING_PERIOD_UNIT_ID"]->SetText(0);
        $this->SQL = "DELETE FROM P_BILLING_PERIOD_UNIT WHERE P_BILLING_PERIOD_UNIT_ID = " . $this->SQLValue($this->cp["P_BILLING_PERIOD_UNIT_ID"]->GetDBValue(), ccsFloat) . "";
...
} //End p_billing_period_unit1DataSource Class @22-FCB6E20C
