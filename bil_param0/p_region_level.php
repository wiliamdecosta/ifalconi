...
//Class_Initialize Event @2-26028742
    function clsGridP_REGION_LEVEL($RelativePath, & $Parent)
    {
        ...
        $this->ADLink->Page = "p_region_level.php";
        ...
        $this->Navigator = & new clsNavigator($this->ComponentName, "Navigator", $FileName, 10, tpMoving, $this,"P_REGION_LEVEL_ID");
        $this->Navigator->PageSizes = array("1", "5", "10", "25", "50");
        $this->P_REGION_LEVEL_Insert = & new clsControl(ccsLink, "P_REGION_LEVEL_Insert", "P_REGION_LEVEL_Insert", ccsText, "", CCGetRequestParam("P_REGION_LEVEL_Insert", ccsGet, NULL), $this);
        $this->P_REGION_LEVEL_Insert->Page = "p_region_level.php";
    }
    ...
    function Show()
    {
        ...
        if (!$this->IsEmpty) {
            ...
            while ($this->ForceIteration || (($this->RowNumber < $this->PageSize) &&  ($this->HasRecord = $this->DataSource->has_next_record()))) {
                ...
                $this->ADLink->Parameters = CCAddParam($this->ADLink->Parameters, "P_REGION_LEVEL_ID", $this->DataSource->f("P_REGION_LEVEL_ID"));
        ...
        $this->P_REGION_LEVEL_Insert->Parameters = CCGetQueryString("QueryString", array("P_REGION_LEVEL_ID", "ccsForm"));
        $this->P_REGION_LEVEL_Insert->Parameters = CCAddParam($this->P_REGION_LEVEL_Insert->Parameters, "FLAG", "ADD");
        $this->Navigator->Show();
        $this->P_REGION_LEVEL_Insert->Show();
        $Tpl->parse();
        $Tpl->block_path = $ParentPath;
...
//DataSource Variables @2-6BC0D0B7
    var $Parent = "";
    var $CCSEvents = "";
    var $CCSEventResult;
    var $ErrorBlock;
...
//DataSourceClass_Initialize Event @2-542A0AF2
...
//Prepare Method @2-4F67FEF9
    function Prepare()
    {
        ...
        $this->wp->Criterion[1] = $this->wp->Operation(opContains, "LEVEL_NAME", $this->wp->GetDBValue("1"), $this->ToSQL($this->wp->GetDBValue("1"), ccsText),false);
...
//Open Method @2-6B57E45C
    function Open()
    {
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildSelect", $this->Parent);
        $this->CountSQL = "SELECT COUNT(*)\n\n" .
        "FROM p_region_level";
        $this->SQL = "SELECT P_REGION_LEVEL_ID, LEVEL_NAME, LEVEL_NUMBER, DESCRIPTION, UPDATE_DATE, UPDATE_BY \n\n" .
...
//SetValues Method @2-EF5B3C8F
    function SetValues()
    {
        ...
        $this->DLink->SetDBValue($this->f("P_REGION_LEVEL_ID"));
        $this->ADLink->SetDBValue($this->f("P_REGION_LEVEL_ID"));
...
class clsRecordp_region_level1 { //p_region_level1 Class @29-C8617981

//Variables @29-D6FF3E86
...
//Class_Initialize Event @29-5919FEF7
    function clsRecordp_region_level1($RelativePath, & $Parent)
    {
        ...
        if($this->Visible)
        {
            ...
            $this->P_REGION_LEVEL_ID = & new clsControl(ccsHidden, "P_REGION_LEVEL_ID", "P_REGION_LEVEL_ID", ccsText, "", CCGetRequestParam("P_REGION_LEVEL_ID", $Method, NULL), $this);
            if(!$this->FormSubmitted) {
...
//CheckErrors Method @29-49763A05
    function CheckErrors()
    {
        $errors = false;
...
} //End p_region_level1 Class @29-FCB6E20C

class clsp_region_level1DataSource extends clsDBConn {  //p_region_level1DataSource Class @29-A8896A89

//DataSource Variables @29-6DDDFDA6
    var $Parent = "";
    var $CCSEvents = "";
    var $CCSEventResult;
    var $ErrorBlock;
...
//DataSourceClass_Initialize Event @29-088E3C1A
    function clsp_region_level1DataSource(& $Parent)
    {
        ...
        $this->P_REGION_LEVEL_ID = new clsField("P_REGION_LEVEL_ID", ccsText, "");
        

    }
//End DataSourceClass_Initialize Event

//Prepare Method @29-8BFC6247
    function Prepare()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
...
//SetValues Method @29-31D20907
    function SetValues()
    {
        ...
        $this->UPDATE_BY->SetDBValue($this->f("UPDATE_BY"));
        $this->UPDATE_DATE->SetDBValue(trim($this->f("UPDATE_DATE")));
        $this->P_REGION_LEVEL_ID->SetDBValue($this->f("P_REGION_LEVEL_ID"));
    }
//End SetValues Method

//Insert Method @29-3EB80A2E
    function Insert()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
        ...
        "VALUES\n" .
        "(GENERATE_ID('TRB','P_REGION_LEVEL','P_REGION_LEVEL_ID'),UPPER(TRIM('" . $this->SQLValue($this->cp["LEVEL_NAME"]->GetDBValue(), ccsText) . "'))," . $this->SQLValue($this->cp["LEVEL_NUMBER"]->GetDBValue(), ccsFloat) . ",'" . $this->SQLValue($this->cp["DESCRIPTION"]->GetDBValue(), ccsText) . "',sysdate,'" . $this->SQLValue($this->cp["UPDATE_BY"]->GetDBValue(), ccsText) . "')";
...
//Delete Method @29-3C23D4B7
    function Delete()
    {
        ...
        if (!strlen($this->cp["P_REGION_LEVEL_ID"]->GetText()) and !is_bool($this->cp["P_REGION_LEVEL_ID"]->GetValue(true))) 
            $this->cp["P_REGION_LEVEL_ID"]->SetText(0);
        $this->SQL = "DELETE FROM P_REGION_LEVEL WHERE P_REGION_LEVEL_ID = " . $this->SQLValue($this->cp["P_REGION_LEVEL_ID"]->GetDBValue(), ccsFloat) . "";
...
} //End p_region_level1DataSource Class @29-FCB6E20C
