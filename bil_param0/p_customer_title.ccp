...
where a.p_title_position_id=b.p_title_position_id and a.p_customer_segment_id=c.p_customer_segment_id
and a.P_CUSTOMER_TITLE_ID={P_CUSTOMER_TITLE_ID}" errorSummator="Error" wizardCaption=" P Customer Title " wizardFormMethod="post" PathID="p_customer_title1" activeCollection="DSQLParameters" parameterTypeListName="ParameterTypeList" customInsertType="SQL" customInsert="INSERT INTO P_CUSTOMER_TITLE(P_CUSTOMER_TITLE_ID, CODE, P_TITLE_POSITION_ID, P_CUSTOMER_SEGMENT_ID, DESCRIPTION, CREATION_DATE, CREATED_BY, UPDATED_DATE, UPDATED_BY) 
VALUES
(GENERATE_ID('TRB','P_CUSTOMER_TITLE','P_CUSTOMER_TITLE_ID'),UPPER(TRIM('{CODE}')),{P_TITLE_POSITION_ID},{P_CUSTOMER_SEGMENT_ID},'{DESCRIPTION}',sysdate,'{CREATED_BY}',sysdate,'{UPDATED_BY}')" customUpdateType="SQL" customUpdate="UPDATE P_CUSTOMER_TITLE SET 
...
<SQLParameter id="96" variable="UPDATED_BY" parameterType="Control" dataType="Text" parameterSource="UPDATED_BY"/>
