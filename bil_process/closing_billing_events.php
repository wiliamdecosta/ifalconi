...
function BindEvents()
{
    ...
    $BATCH1->BATCH_Close->CCSEvents["BeforeShow"] = "BATCH1_BATCH_Close_BeforeShow";
...
//DEL  // -------------------------
//DEL      // Write your own code here.
//DEL  	if($BATCH1->EditMode)
//DEL  		$BATCH1->Button_Close->Visible=true;
//DEL  	else
//DEL  		$BATCH1->Button_Close->Visible=false;
//DEL  // -------------------------

//BATCH1_BATCH_Close_BeforeShow @395-56617171
function BATCH1_BATCH_Close_BeforeShow(& $sender)
{
    $BATCH1_BATCH_Close_BeforeShow = true;
    $Component = & $sender;
    $Container = & CCGetParentContainer($sender);
    global $BATCH1; //Compatibility
//End BATCH1_BATCH_Close_BeforeShow
...
//Close BATCH1_BATCH_Close_BeforeShow @395-197D760C
    return $BATCH1_BATCH_Close_BeforeShow;
}
//End Close BATCH1_BATCH_Close_BeforeShow
...
// -------------------------
	...
	$V_INPUT_DATA_CONTROL_BILL->BATCH_Insert1->Parameters = CCRemoveParam($V_INPUT_DATA_CONTROL_BILL->BATCH_Insert1->Parameters, "INPUT_DATA_CONTROL_ID");
...
//Page_BeforeShow @1-8B99F3D4
function Page_BeforeShow(& $sender)
{
    $Page_BeforeShow = true;
    $Component = & $sender;
    $Container = & CCGetParentContainer($sender);
    global $closing_billing; //Compatibility
//End Page_BeforeShow
...
//Page_BeforeInitialize @1-BF27BEE0
...
//Custom Code @399-2A29BDB7
// -------------------------
    // Write your own code here.
	if(CCGetFromGET('actionClose')==true){
		$addQueryString = CCGetQueryString('QueryString');
		$addQueryString = CCRemoveParam($addQueryString,"actionClose");
		
		$dbConn = new clsDBConn();
	 	$query = "SELECT F_SUBMIT_JOB('CLOSE_BILL_PROCESS'," . CCGetFromGet("INPUT_DATA_CONTROL_ID", "0") . ",'" . CCGetSession("UserName", "-") . "') as hasil from dual";
		$dbConn->query($query);
		$dbConn->next_record();
		$hasil = $dbConn->Record;
		$msg = $hasil[HASIL];
		//echo strpos($hasil[HASIL],'ERROR') !== false;
		//print_r($hasil[HASIL]);
		//exit;

		if (strpos($hasil[HASIL],'ERROR') !== false)
		  {
		      ?>
